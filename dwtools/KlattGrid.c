...
Sound Sound_KlattGrid_filter_cascade_laryngial (Sound me, KlattGrid thee);
Sound Sound_KlattGrid_filter_parallel_frication (Sound me, KlattGrid thee);
Sound Sound_KlattGrid_filter_parallel_laryngial (Sound me, KlattGrid thee);

int KlattGrid_replaceFormantGrid (KlattGrid me, FormantGrid thee);
Sound KlattGrid_to_Sound_frication (KlattGrid me, double samplingFrequency);

// Maximum amplitue (-1,1) at 91 dB
#define AMPLITUDE_dBMax 91

#define KLATTGRID_debug 1
...
Sound PhonationGrid_to_Sound_voiced (PhonationGrid me, double samplingFrequency, double maximumPeriod, RealTier df1, RealTier db1, RealTier *df1_micro, RealTier *db1_micro, int klatt88)
{
	...
	Sound thee = Sound_create (1, my xmin, my xmax, sound_nt, dt, sound_t1);
	...
	/*
		Spectral tilt by means of filter y[n]=a x[n]+b y[n]
		|H(f)| = a / sqrt(1-2*b*cos(2*pi*f*T)+b^2) for b <= 1
		|H(0)|= a /(1 - b) => a = 1 - b
		Now solve 20 log|H(F)|= -c (for F=3 kHz and c>0)
		Solution: for q = (1 - D * cos(2*pi*F*T)) / (1 - D), where D = 10^(-c/10)
			b = q -sqrt(q^2 - 1)
	*/

	double cosf = cos (2 * NUMpi * 3000 * thy dx), ynm1 = 0;
	for (i = 1; i <= thy nx; i++)
	{
		// First: av
...
int Sound_FormantGridP_filterWithOneFormant_inline (Sound me, FormantGridP thee, long iformant, RealTier dftier, RealTier dbtier)
{
	...
	for (long is = 1; is <= my nx; is++)
	{
		...
		if (f <= nyquist && b != NUMundefined && a != NUMundefined)
		{
...
Sound Sound_KlattGrid_filter_parallel_laryngial (Sound me, KlattGrid thee)
{
	int alternatingSign = 0; // No alternating signs in parallel adding of filter outputs
	long is;
	...
	Sound him = Sound_FormantGridP_filter (me, thy nasal_formants, 1, numberOfNasalFormants, alternatingSign);
	if (him == NULL) return NULL;

	// TODO: df and db tiers ???
	Sound f1 = Sound_FormantGridP_filter (me, thy formants, 1, 1, alternatingSign);
	if (f1 == NULL) goto end;
	
	for (is = 1; is <= my nx; is++)
	{
		his z[1][is] += f1 -> z[1][is];
	}

	// Forants 2 and up, with alternating signs.
	// We perform the pre-emphasis by differentiating afterwards in the summation step.
	
	alternatingSign = -1; // start with negative sign
	Sound vocalTract = Sound_FormantGridP_filter (me, thy formants, 2, numberOfFormants, alternatingSign);
	if (vocalTract == NULL) goto end;

	alternatingSign = 0;
	Sound trachea =  Sound_FormantGridP_filter (me, thy tracheal_formants, 2, numberOfTrachealFormants, alternatingSign);
	if (trachea == NULL) goto end;

	// Sum the nasal + formant + tracheal parts

	double pval = 0;
	for (is = 1; is <= my nx; is++)
	{
		double val =  vocalTract -> z[1][is] - trachea -> z[1][is];
		his z[1][is] += val - pval; // pre-emphasis
		pval = val;
	}
	
end:
	forget (vocalTract); forget (f1); forget (trachea);
	if (Melder_hasError ()) forget (him);
	return him;	
}
...
Sound Sound_KlattGrid_filter_parallel_frication (Sound me, KlattGrid thee)
{
	...
	long ifb = 2;
	long ife = thy klatt88 ? (fgp -> formants -> size > 6 ? 6 : fgp -> formants -> size) : fgp -> formants -> size;
	int alternatingSign = 1; // start with +
	
	Sound him = Sound_FormantGridP_filter (me, fgp, ifb, ife, alternatingSign);
	if (him == NULL) return NULL;

	// Bypass
	for (long is = 1; is <= his nx; is++)
	{
		double t = his x1 + (is - 1) * his dx;
		double ab = RealTier_getValueAtTime (thy bypassAmplitude, t);
		his z[1][is] += my z[1][is] * DB_to_A (ab);
	}
	return him;
}
...
Sound Sound_KlattGrid_filter_cascade_laryngial (Sound me, KlattGrid thee)
{
	...
	long iformant; int antiformant = 0;
	...
	// Nasal formants
	antiformant = 0;
	for (iformant = 1; iformant <= numberOfNasalFormants; iformant++)
	{
		_Sound_FormantGrid_filterWithOneFormant_inline (him, nasal_formants, iformant, antiformant);
	}
	
	// Nasal anti formants
	antiformant = 1;
	for (iformant = 1; iformant <= numberOfNasalAntiFormants; iformant++)
	{
		_Sound_FormantGrid_filterWithOneFormant_inline (him, nasal_antiformants, iformant, antiformant);
	}
	
	// Tracheal formants
	
	antiformant = 0;
	for (iformant = 1; iformant <= numberOfTrachealFormants; iformant++)
	{
		_Sound_FormantGrid_filterWithOneFormant_inline (him, tracheal_formants, iformant, antiformant);
	}
	
	// Tracheal anti formants
	antiformant = 1;
	for (iformant = 1; iformant <= numberOfTrachealAntiFormants; iformant++)
	{
		_Sound_FormantGrid_filterWithOneFormant_inline (him, tracheal_antiformants, iformant, antiformant);
	}
	...
	{
		_Sound_FormantGrid_filterWithOneFormant_inline (him, formants, iformant, antiformant);
	}
	return him;
}
...
Sound KlattGrid_to_Sound_frication (KlattGrid me, double samplingFrequency)
{
	...
	for (i = 1; i <= nt; i++)
	{
		double t = thy x1 + (i - 1) * dt;
		double val = NUMrandomUniform (-1, 1);
		double a = RealTier_getValueAtTime (my fricationAmplitude, t);
		thy z[1][i] = val * DB_to_A (a);
	}

	Sound him = Sound_KlattGrid_filter_parallel_frication (thee, me);
	forget (thee);
	return him;	
}
...
KlattGrid KlattGrid_createExample (void)
{
	...
	double tmax = nrows * frameDuration;
	double ap[7] = {0, 0.4, 0.15, 0.06, 0.04, 0.022, 0.03 };

	me = KlattGrid_create (0, tmax);
	if (me == NULL) goto end;
	my klatt88 = 1;
	for (long irow = 1; irow <= nrows; irow++)
	{
		double val, f0, fk, t = (irow - 1) * frameDuration;
		long icol = 1;
...
//		RealTier_addPoint (my source -> amplitudeOfVoicing, t, val);
		...
		if (val < 13) val = dBNul; else val += 20 * log (0.05);
		...
		if (val < 13) val = dBNul; else val += 20 * log (0.1);
		...
		if (val < 13) val = dBNul; else val += 20 * log (0.25);
		...
		for (long kf = 1; kf <= 6; kf++)
		{
			...
			if (val < 13) val = dBNul; else val += 20 * log (ap[kf]);
			RealTier_addPoint (my formants -> amplitudes -> item[kf], t, val);
			icol++; val = Table_getNumericValue (kt, irow, icol); // Bhz
			RealTier_addPoint (my frication_formants -> formants -> item[kf], t, val);
		}
		
		icol++; val = Table_getNumericValue (kt, irow, icol); // ANP
		if (val < 13) val = dBNul; else val += 20 * log (0.6);
		...
		RealTier_addPoint (my gain, t, val);
	}
end:
	forget (kt);
	if (Melder_hasError ()) forget (me);
