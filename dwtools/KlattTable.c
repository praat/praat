...
static void KlattGlobal_getFrame (KlattGlobal me, KlattFrame thee)
{
	...
	amp_parFNP = DBtoLIN (thy ANP) * 0.6;
...
{
	long itemp, current_value, next_value;
	double ftemp, result = 0, diff_value, temp_diff;
	...
	if(my T0 != 0)
	{
		...
		temp_diff = ftemp - itemp;

		current_value = my natural_samples[itemp];
		next_value = my natural_samples[itemp+1];

		diff_value = next_value - current_value;
...
static void KlattGlobal_synthesizeFrame (KlattGlobal me, short *output) 
{
	...
	long i, n4;
	double frics, glotout, aspiration, par_glotout, noise, sourc, voice;
	...
	for (my ns = 0; my ns < my nspfr; my ns++)
	{
		...
		*/
		...
		temp = out * my amp_gain0;  /* Convert back to integer */
