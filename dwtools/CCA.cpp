...
CCA TableOfReal_to_CCA (TableOfReal me, long ny)
{
	try {
		...
		if (ny < 1 || ny > my numberOfColumns - 1) Melder_throw ("Dimension of first part not correct.");
		...
		if (fnormy == 0 || fnormx == 0) Melder_throw ("One of the parts of the table contains only zeros.");
...
TableOfReal CCA_and_TableOfReal_predict (CCA me, TableOfReal thee, long from)
{
	try {
		...
		*/
		...
		if (ny != nev) Melder_throw ("There are not enough correlations present for prediction.");
	
		if (from == 0) from = 1;
		long ncols = thy numberOfColumns - from + 1;
		if (from < 1 || ncols != nx) Melder_throw ("The number of columns to analyze must be equal to ",nx, "."); 
