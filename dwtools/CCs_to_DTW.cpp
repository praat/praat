...
DTW CCs_to_DTW (I, thou, double wc, double wle, double wr, double wer, double dtr, int matchStart, int matchEnd, int constraint)
{
	try {
		...
		if (my maximumNumberOfCoefficients != thy maximumNumberOfCoefficients) Melder_throw ("CC orders must be equal.");
		long nr = dtr / my dx;
		if (wr != 0 && nr < 2)Melder_throw ("Time window for regression is too small.");
		
		if (nr % 2 == 0) nr++;
		if (wr != 0) Melder_casual ("%ld frames used for regression coefficients.", nr);
		...
		for (long i = 1; i <= my nx; i++)
		{
			...
			if ((i % 10) == 1) { 
				Melder_progress5 (0.999 * i / my nx, L"Calculate distances: frame ", Melder_integer (i), L" from ", Melder_integer (my nx), L"."); therror
			}
		}
