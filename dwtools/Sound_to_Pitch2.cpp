	...
	double compressionFactor, long nPointsPerOctave)
{
	try {
		...
		autoNUMvector<double> y2 (1, nfft2);
		...
		*/
		...
		for (long i = 2; i <= nfft2; i++) fl2[i] = NUMlog2 ((i-1) * df);
		fl2[1] = 2 * fl2[2] - fl2[3];
		...
		for (long i = 1; i <= numberOfFrames; i++)
		{
			...
			NUMspline (fl2.peek(), specAmp.peek(), nfft2, 1e30, 1e30, y2.peek());
			for (long j=1; j <= nFrequencyPoints; j++)
			{
				double f = fminl2 + (j-1) * dfl2;
				NUMsplint (fl2.peek(), specAmp.peek(), y2.peek(), nfft2, f, &al2[j]);
			}
			...
			*/
			...
			if (f0 > 0) cc[i] = Sound_correlateParts (sound.peek(), tmid - 1.0 / f0, tmid, 1.0 / f0);
		}
