...
int Pattern_init (I, long ny, long nx)
{
	try {
		...
		my ny = ny;
		my nx = nx;
		Matrix_init (me, 1, nx, nx, 1, 1, 1, ny, ny, 1, 1); therror
		return 1;
...
void Pattern_normalize (I, int choice, double pmin, double pmax)
{
    ...
    if (choice == 1)
    {
		for (long i = 1; i <= my ny; i++)
		{
			for (long j = 1; j <= my nx; j++) { my z[i][j] = ( my z[i][j] - pmin) / ( pmax - pmin); }
		}
    }
    else /* default choice */
    {
		...
		{
			double sum = 0;
			for (long j=1; j <= my nx; j++) { sum += ( my z[i][j] -= pmin); }
			for (long j=1; j <= my nx; j++) { my z[i][j] *= 1.0 / sum; }
		}
	}
}
