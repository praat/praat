...
MAN_BEGIN (U"solving matrix equations", U"djmw", 20191217)
...
NORMAL (U"As a check we can calculate the norm of the difference between ##A\\.cx# and #y which should be zero for a perfect solution.")
CODE (U"appendInfoLine: norm (mul\\#  (a\\# \\# , x\\# ) - y\\# )")
...
NORMAL (U"If the matrix has more rows than colums an exact solution is generally not possible and the best thing we can do is to find a solution vector #x that %%minimizes% the squared distance between the vectors ##A\\.cx# and #y. The problem now can be posed as: find the vector #x that minimizes ||##A\\.cx# - #y||^2. This is a regression problem which can be solved using @@singular value decomposition@.")
NORMAL (U"The following example shows a 5\\xx2 matrix #A. The solution therefore is two dimensional.")
CODE (U"a\\# \\#  = {{-0.84,-0.184},")
CODE (U"...    { 0.09, 1.26},")
CODE (U"...    { 0.62,-0.20},")
CODE (U"...    {-1.48,-1.03},")
...
NORMAL (U"This means that no other vector #x can be found that has ||##A\\.cx# - #y|| < 2.556992185298919!")
...
NORMAL (U"If there are additional constraints on the vector #x we can handle two different cases:")
...
NORMAL (U"The function is called \"weakly constrained\" because the penalty function prohibits a relatively large departure of ##x\\'px# from \\de. If we let \\al go to infinity we have a constrained least squares regression problem of minimizing ||##A\\.cx# - #y||^^2^ subject to ##x\\'px# = \\de. The algorithm is due to @@Ten Berge (1991)@.")
...
NORMAL (U"Here we constrain the elements of the solution to be nonnegative. The problem is stated as %minimize ||A\\.cx -y||^2 where all %x__%i_ \\>_ 0.")
NORMAL (U"This problem can be solved by an iterative alternating least squares method described by @@Borg & Groenen (1997)@. The function to use is ##solveNonnegative\\#  (a\\# \\# , y\\#  [,x\\# ], %maximumNumberOfIterations, %tolerance, %info)#")
...
NORMAL (U"If the matrix #A has some special properties #and the solution vector has to be sparse, i.e. most of its values are zero, then we can find the #x that minimizes ||##A\\.cx# - #y||^2.")
...
TAG (U"##info")
DEFINITION (U"determines whether information is written to the info window during iterations. No info is written if the value is zero.")
...
CODE (U"  x\\#  [i] = if randomUniform (0,1) < 0.005 then randomUniform (0.1, 10) else 0.0 fi")
CODE (U"endfor")
