...
Pitch SPINET_to_Pitch (SPINET me, double harmonicFallOffSlope, double ceiling, int maxnCandidates)
{
	try {
		...
		if (nFrequencyPoints < 2) Melder_throw ("Frequency range too small.");
		if (ceiling <= fmin) Melder_throw ("Ceiling is smaller than centre frequency of lowest filter.");
		...
		autoNUMvector<double> y2 (1, my ny);
		...
		{
			double p = 0;
			for (long i = 1; i <= my ny; i++) p += my s[i][j];
			if (p > maxPower) maxPower = p;
			power[j] = p;
		}
		if (maxPower == 0) Melder_throw ("No power");
		...
		for (long j = 1; j <= my nx; j++)
		{
			...
			for (long i = 1; i <= my ny; i++) y[i] = my s[i][j];
			NUMspline (fl2.peek(), y.peek(), my ny, 1e30, 1e30, y2.peek()); 
			for (long k = 1; k <= nFrequencyPoints; k++)
			{
				double f = fminl2 + (k-1) * dfl2;
				NUMsplint (fl2.peek(), y.peek(), y2.peek(), my ny, f, & pitch[k]);
				sumspec[k] = 0;
			}
