...
//#include "FormantAnalyzer_enums.h"
//#include "enums_getValue.h"
//#include "FormantAnalyzer_enums.h"

Thing_implement (FormantAnalyzer, TimeSoundAnalysisEditor, 0);
...
void structFormantAnalyzer :: v_info () {
	FormantAnalyzer_Parent :: v_info ();
	MelderInfo_writeLine (U"FormantModeler minimum ceiling: ", our minimumCeiling_Hz, U" Hz");
	MelderInfo_writeLine (U"FormantModeler maximum ceiling: ", our maximumCeiling_Hz, U" Hz");
	MelderInfo_writeLine (U"FormantModeler number of models: ", our numberOfModels);
	MelderInfo_writeLine (U"FormantModeler number of parameters for F1 track: ", our numberOfParametersF1Track);
	MelderInfo_writeLine (U"FormantModeler number of parameters for F2 track: ", our numberOfParametersF2Track);
	MelderInfo_writeLine (U"FormantModeler number of parameters for F3 track: ", our numberOfParametersF3Track);
	MelderInfo_writeLine (U"FormantModeler number of parameters for F4 track: ", our numberOfParametersF4Track);
	MelderInfo_writeLine (U"FormantModeler number of parameters for F5 track: ", our numberOfParametersF5Track);
}

/********** UTILITIES **********/

static double _FormantAnalyzer_computeSoundY (FormantAnalyzer me) {
...
static integer _FormantAnalyzer_yWCtoTier (FormantAnalyzer me, double yWC) {
	const TextGrid grid = (TextGrid) my data;
	const integer numberOfTiers = grid -> tiers->size;
	const double soundY = _FormantAnalyzer_computeSoundY (me);
...
static void _FormantAnalyzer_timeToInterval (FormantAnalyzer me, double t, integer tierNumber,
...
static void checkTierSelection (FormantAnalyzer me, conststring32 verbPhrase) {
	const TextGrid grid = (TextGrid) my data;
...
static integer getSelectedInterval (FormantAnalyzer me) {
	const TextGrid grid = (TextGrid) my data;
...
static integer getSelectedLeftBoundary (FormantAnalyzer me) {
	const TextGrid grid = (TextGrid) my data;
	Melder_assert (my selectedTier >= 1 || my selectedTier <= grid -> tiers->size);
	const IntervalTier tier = (IntervalTier) grid -> tiers->at [my selectedTier];
	Melder_assert (tier -> classInfo == classIntervalTier);
...
static integer getSelectedPoint (FormantAnalyzer me) {
	const TextGrid grid = (TextGrid) my data;
	Melder_assert (my selectedTier >= 1 || my selectedTier <= grid -> tiers->size);
...
static void scrollToView (FormantAnalyzer me, double t) {
	if (t <= my startWindow) {
...
/*
 * The main invariant of the FormantAnalyzer is that the selected interval
...
static void menu_cb_ExtractSelectedTextGrid_preserveTimes (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_ExtractSelectedTextGrid_timeFromZero (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	if (my endSelection <= my startSelection)
		Melder_throw (U"No selection.");
...
void structFormantAnalyzer :: v_createMenuItems_file_extract (EditorMenu menu) {
	FormantAnalyzer_Parent :: v_createMenuItems_file_extract (menu);
...
static void menu_cb_WriteToTextFile (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
void structFormantAnalyzer :: v_createMenuItems_file_write (EditorMenu menu) {
	FormantAnalyzer_Parent :: v_createMenuItems_file_write (menu);
	EditorMenu_addCommand (menu, U"Save TextGrid as text file...", 'S', menu_cb_WriteToTextFile);
}

static void menu_cb_DrawVisibleTextGrid (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_DrawVisibleSoundAndTextGrid (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
void structFormantAnalyzer :: v_createMenuItems_file_draw (EditorMenu menu) {
	FormantAnalyzer_Parent :: v_createMenuItems_file_draw (menu);
...
#ifndef macintosh
static void menu_cb_Cut (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	GuiText_cut (my text);
}
static void menu_cb_Copy (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	GuiText_copy (my text);
}
static void menu_cb_Paste (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	GuiText_paste (my text);
}
static void menu_cb_Erase (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	GuiText_remove (my text);
}
#endif

static void menu_cb_ConvertToBackslashTrigraphs (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_ConvertToUnicode (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_GetStartingPointOfInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_GetEndPointOfInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_GetLabelOfInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void do_selectAdjacentTier (FormantAnalyzer me, bool previous) {
	const TextGrid grid = (TextGrid) my data;
	...
	if (n >= 2) {
		...
		_FormantAnalyzer_timeToInterval (me, my startSelection, my selectedTier, & my startSelection, & my endSelection);
		FunctionEditor_marksChanged (me, true);
	}
}

static void menu_cb_SelectPreviousTier (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_selectAdjacentTier (me, true);
}

static void menu_cb_SelectNextTier (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_selectAdjacentTier (me, false);
}

static void do_selectAdjacentInterval (FormantAnalyzer me, bool previous, bool shift) {
	const TextGrid grid = (TextGrid) my data;
	IntervalTier intervalTier;
	TextTier textTier;
	if (my selectedTier < 1 || my selectedTier > grid -> tiers->size)
...
static void menu_cb_SelectPreviousInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_selectAdjacentInterval (me, true, false);
}

static void menu_cb_SelectNextInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_selectAdjacentInterval (me, false, false);
}

static void menu_cb_ExtendSelectPreviousInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_selectAdjacentInterval (me, true, true);
}

static void menu_cb_ExtendSelectNextInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_selectAdjacentInterval (me, false, true);
}

static void menu_cb_MoveBtoZero (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_MoveCursorToZero (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_MoveEtoZero (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_DrawTextGridAndPitch (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void insertBoundaryOrPoint (FormantAnalyzer me, integer itier, double t1, double t2, bool insertSecond) {
	const TextGrid grid = (TextGrid) my data;
	const integer numberOfTiers = grid -> tiers->size;
		...
		if (insertSecond && numberOfTiers >= 2 && t1 == t2) {
			...
			*/
			...
			for (integer jtier = 1; jtier <= numberOfTiers; jtier ++) {
				if (jtier != itier) {
					double tmin, tmax;
					_FormantAnalyzer_timeToInterval (me, t1, jtier, & tmin, & tmax);
					if (tmin > tlast)
						tlast = tmin;
				}
			}
...
static void do_insertIntervalOnTier (FormantAnalyzer me, int itier) {
	try {
...
static void menu_cb_InsertIntervalOnTier1 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 1); }
static void menu_cb_InsertIntervalOnTier2 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 2); }
static void menu_cb_InsertIntervalOnTier3 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 3); }
static void menu_cb_InsertIntervalOnTier4 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 4); }
static void menu_cb_InsertIntervalOnTier5 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 5); }
static void menu_cb_InsertIntervalOnTier6 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 6); }
static void menu_cb_InsertIntervalOnTier7 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 7); }
static void menu_cb_InsertIntervalOnTier8 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertIntervalOnTier (me, 8); }

static void menu_cb_AlignInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_AlignmentSettings (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_RemovePointOrBoundary (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void do_movePointOrBoundary (FormantAnalyzer me, int where) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_MoveToB (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_movePointOrBoundary (me, 1);
}

static void menu_cb_MoveToE (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_movePointOrBoundary (me, 2);
}

static void menu_cb_MoveToZero (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_movePointOrBoundary (me, 0);
}

static void do_insertOnTier (FormantAnalyzer me, integer itier) {
	try {
		insertBoundaryOrPoint (me, itier,
			my playingCursor || my playingSelection ? my playCursor : my startSelection,
			my playingCursor || my playingSelection ? my playCursor : my endSelection,
...
static void menu_cb_InsertOnSelectedTier (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_insertOnTier (me, my selectedTier);
}

static void menu_cb_InsertOnTier1 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 1); }
static void menu_cb_InsertOnTier2 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 2); }
static void menu_cb_InsertOnTier3 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 3); }
static void menu_cb_InsertOnTier4 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 4); }
static void menu_cb_InsertOnTier5 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 5); }
static void menu_cb_InsertOnTier6 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 6); }
static void menu_cb_InsertOnTier7 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 7); }
static void menu_cb_InsertOnTier8 (FormantAnalyzer me, EDITOR_ARGS_DIRECT) { do_insertOnTier (me, 8); }

static void menu_cb_InsertOnAllTiers (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void findInTier (FormantAnalyzer me) {
	const TextGrid grid = (TextGrid) my data;
...
static void do_find (FormantAnalyzer me) {
	if (my findString) {
		integer left, right;
...
static void menu_cb_formantModelerSettings (FormantAnalyzer me, EDITOR_ARGS_FORM) {
	EDITOR_FORM (U"Formant modeler settings", nullptr)
		...
		POSITIVE (maximumCeiling_Hz, U"Maximum ceiling (Hz)", my default_modeler_maximumCeiling_Hz ())
		NATURAL (numberOfModels, U"Number of models", my default_modeler_numberOfModels ())
	...
	EDITOR_OK
		SET_REAL (minimumCeiling_Hz,    my p_modeler_minimumCeiling_Hz)
		SET_REAL (maximumCeiling_Hz,    my p_modeler_maximumCeiling_Hz)
		SET_INTEGER (numberOfModels, my p_modeler_numberOfModels)
	...
	EDITOR_DO
		Melder_require (minimumCeiling_Hz <= maximumCeiling_Hz,
			U"The \"Minimum ceiling\" should not exceed the \"Maximum ceiling\".");
		double nyquistFrequency = 0.5 / my d_sound.data -> dx;
		Melder_require (maximumCeiling_Hz <= nyquistFrequency,
			U"The \"maximumCeiling\" should not exceed the Nyquits frequency (", nyquistFrequency, U" Hz).");
		//my pref_formant_maximumFormant   () = my p_formant_maximumFormant   = maximumFormant;
		my pref_modeler_minimumCeiling_Hz () = my p_modeler_minimumCeiling_Hz = minimumCeiling_Hz;
		my pref_modeler_maximumCeiling_Hz () = my p_modeler_maximumCeiling_Hz = maximumCeiling_Hz;
		my pref_modeler_numberOfModels () = my p_modeler_numberOfModels = numberOfModels;
...
static void menu_cb_Find (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_FindAgain (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	do_find (me);
}

static void checkSpellingInTier (FormantAnalyzer me) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_CheckSpelling (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
...
static void menu_cb_CheckSpellingInInterval (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	if (my spellingChecker) {
		integer left, right;
		autostring32 label = GuiText_getStringAndSelectionPosition (my text, & left, & right);
		integer position = right;
...
static void menu_cb_AddToUserDictionary (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	if (my spellingChecker) {
...
static void menu_cb_RenameTier (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_PublishTier (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_RemoveAllTextFromTier (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_RemoveTier (FormantAnalyzer me, EDITOR_ARGS_DIRECT) {
	const TextGrid grid = (TextGrid) my data;
...
static void menu_cb_AddIntervalTier (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_AddPointTier (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_DuplicateTier (FormantAnalyzer me, EDITOR_ARGS_FORM) {
...
static void menu_cb_FormantAnalyzerHelp (FormantAnalyzer, EDITOR_ARGS_DIRECT) { Melder_help (U"FormantAnalyzer"); }
static void menu_cb_AboutSpecialSymbols (FormantAnalyzer, EDITOR_ARGS_DIRECT) { Melder_help (U"Special symbols"); }
static void menu_cb_PhoneticSymbols (FormantAnalyzer, EDITOR_ARGS_DIRECT) { Melder_help (U"Phonetic symbols"); }
static void menu_cb_AboutTextStyles (FormantAnalyzer, EDITOR_ARGS_DIRECT) { Melder_help (U"Text styles"); }

void structFormantAnalyzer :: v_createMenus () {
	FormantAnalyzer_Parent :: v_createMenus ();
	//EditorMenu menu;
...
void structFormantAnalyzer :: v_createHelpMenuItems (EditorMenu menu) {
	FormantAnalyzer_Parent :: v_createHelpMenuItems (menu);
	EditorMenu_addCommand (menu, U"FormantAnalyzer help", '?', menu_cb_FormantAnalyzerHelp);
...
static void gui_text_cb_changed (FormantAnalyzer me, GuiTextEvent /* event */) {
	const TextGrid grid = (TextGrid) my data;
...
void structFormantAnalyzer :: v_createChildren () {
	FormantAnalyzer_Parent :: v_createChildren ();
	if (our text)
		GuiText_setChangedCallback (our text, gui_text_cb_changed, this);
}

void structFormantAnalyzer :: v_dataChanged () {
	...
	*/
	...
	FormantAnalyzer_Parent :: v_dataChanged ();   // does all the updating
}

/********** DRAWING AREA **********/

void structFormantAnalyzer :: v_prepareDraw () {
...
static void do_drawIntervalTier (FormantAnalyzer me, IntervalTier tier, integer itier) {
...
static void do_drawTextTier (FormantAnalyzer me, TextTier tier, integer itier) {
	#if gtk || defined (macintosh)
		constexpr bool platformUsesAntiAliasing = true;
	#else
		constexpr bool platformUsesAntiAliasing = false;
...
void structFormantAnalyzer :: v_draw () {
	...
	const double soundY = _FormantAnalyzer_computeSoundY (this), soundY2 = showAnalysis ? 0.5 * (1.0 + soundY) : soundY;
...
void structFormantAnalyzer :: v_drawSelectionViewer () {
...
static void do_drawWhileDragging (FormantAnalyzer me, double numberOfTiers, bool selectedTier [], double x, double soundY) {
...
static void do_dragBoundary (FormantAnalyzer me, double xbegin, integer iClickedTier, int shiftKeyPressed) {
	const TextGrid grid = (TextGrid) my data;
	const integer numberOfTiers = grid -> tiers->size;
	...
	*/
	const integer itierDrop = _FormantAnalyzer_yWCtoTier (me, yWC);
...
bool structFormantAnalyzer :: v_click (double xclick, double yWC, bool shiftKeyPressed) {
	const TextGrid grid = (TextGrid) our data;
	...
	*/
	const double soundY = _FormantAnalyzer_computeSoundY (this);
	if (yWC > soundY) {   // clicked in sound part?
		...
		our FormantAnalyzer_Parent :: v_click (xclick, yWC, shiftKeyPressed);
	...
	*/
	const integer clickedTierNumber = _FormantAnalyzer_yWCtoTier (this, yWC);
	...
	double tmin, tmax;
	_FormantAnalyzer_timeToInterval (this, xclick, clickedTierNumber, & tmin, & tmax);
	IntervalTier intervalTier;
	TextTier textTier;
...
bool structFormantAnalyzer :: v_clickB (double t, double yWC) {
	const double soundY = _FormantAnalyzer_computeSoundY (this);
	if (yWC > soundY) {   // clicked in sound part?
		if (t < our endWindow) {
	...
	const integer clickedTierNumber = _FormantAnalyzer_yWCtoTier (this, yWC);
	double tmin, tmax;
	_FormantAnalyzer_timeToInterval (this, t, clickedTierNumber, & tmin, & tmax);
...
bool structFormantAnalyzer :: v_clickE (double t, double yWC) {
	const double soundY = _FormantAnalyzer_computeSoundY (this);
	if (yWC > soundY) {   // clicked in sound part?
		our endSelection = t;
		if (our startSelection > our endSelection)
...
void structFormantAnalyzer :: v_clickSelectionViewer (double xWC, double yWC) {
	const TextGrid grid = (TextGrid) our data;
...
void structFormantAnalyzer :: v_play (double tmin, double tmax) {
	if (! d_sound.data && ! d_longSound.data)
		return;
...
void structFormantAnalyzer :: v_updateText () {
	const TextGrid grid = (TextGrid) our data;
...
*/
void structFormantAnalyzer :: v_prefs_addFields (EditorCommand cmd) {
	structTextGridEditor :: v_prefs_addFields (cmd);
}


void structFormantAnalyzer :: v_prefs_setValues (EditorCommand cmd) {
	structTextGridEditor :: v_prefs_setValues (cmd);
}

void structFormantAnalyzer :: v_prefs_getValues (EditorCommand cmd) {
	structTextGridEditor :: v_prefs_getValues (cmd);
}

void structFormantAnalyzer :: v_createMenuItems_view_timeDomain (EditorMenu menu) {
	FormantAnalyzer_Parent :: v_createMenuItems_view_timeDomain (menu);
...
void structFormantAnalyzer :: v_highlightSelection (double left, double right, double bottom, double top) {
	if (our v_hasAnalysis () && our p_spectrogram_show && (our d_longSound.data || our d_sound.data)) {
		const double soundY = _FormantAnalyzer_computeSoundY (this), soundY2 = 0.5 * (1.0 + soundY);
...
void structFormantAnalyzer :: v_unhighlightSelection (double left, double right, double bottom, double top) {
	if (our v_hasAnalysis () && our p_spectrogram_show && (our d_longSound.data || our d_sound.data)) {
		const double soundY = _FormantAnalyzer_computeSoundY (this), soundY2 = 0.5 * (1.0 + soundY);
...
double structFormantAnalyzer :: v_getBottomOfSoundArea () {
	return _FormantAnalyzer_computeSoundY (this);
}

double structFormantAnalyzer :: v_getBottomOfSoundAndAnalysisArea () {
	return _FormantAnalyzer_computeSoundY (this);
}

void structFormantAnalyzer :: v_createMenuItems_pitch_picture (EditorMenu menu) {
	FormantAnalyzer_Parent :: v_createMenuItems_pitch_picture (menu);
...
autoFormantAnalyzer FormantAnalyzer_create (conststring32 title, Sampled sound, bool ownSound, Formant formant, TextGrid grid, SpellingChecker spellingChecker, conststring32 callbackSocket) {
	try {
		autoFormantAnalyzer me = Thing_new (FormantAnalyzer);
		FormantAnalyzer_init (me.get(), title, sound, ownSound, formant, grid, spellingChecker, callbackSocket);
		return me;
	} catch (MelderError) {
		Melder_throw (U"FormantAnalyzer window not created.");
	}
}

/* End of file FormantAnalyzer.cpp */
