...
void Matrix_scale (I, int choice)
{
	...
	if (choice == 2) /* by row */
	{
		for (long i = 1; i <= my ny; i++)
		{
			...
			else for (long j = 1; j <= my nx; j++) my z[i][j] /= extremum;
		}
	}
	else if (choice == 3) /* by col */
	{
		for (long j = 1; j <= my nx; j++)
		{
			...
			extremum =  fabs (max) > fabs (min) ? fabs (max) : fabs (min);
			if (extremum == 0.0) nZero++;
			else for (long i = 1; i <= my ny; i++) my z[i][j] /= extremum;
		}
	}
	else if (choice == 1) /* overall */
	{
		...
		else
		{
			for (long i = 1; i <= my ny; i++)
			{
				for (long j = 1; j <= my nx; j++) my z[i][j] /= extremum;
			}
		}
	}
	else
...
Matrix Matrix_solveEquation (I, double tolerance)
{
	try {
		...
		if (nc == 0) Melder_throw ("Matrix_solveEquation: there must be at least 2 columns in the matrix.");
		if (nr < nc) Melder_warning1 (L"Matrix_solveEquation: solution is not unique (fewer equations than unknowns).");
		...
		for (long i = 1; i <= nr; i++)
		{
			for (long j = 1; j <= nc; j++) u[i][j] = my z[i][j];
			b[i] = my z[i][my nx];
		}
