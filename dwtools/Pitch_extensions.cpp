	...
	double pitchMin_Hz, double pitchMax_Hz, int pitchUnit)
{
	try {
		...
		return thee.transfer();
	} catch (MelderError) { rethrowmzero ("PitchTier not created."); }
}

Pitch PitchTier_to_Pitch (PitchTier me, double dt, double pitchFloor, double pitchCeiling)
{
	try {
		if (my points -> size < 1) Melder_throw ("The PitchTier is empty.");
		if (dt <= 0) Melder_throw ("The time step should be a positive number.");
		if (pitchFloor >= pitchCeiling) Melder_throw ("The pitch ceiling must be a higher number than the pitch floor.");
		double tmin = my xmin, tmax = my xmax, t1 = my xmin + dt / 2;
		long nt = (tmax - tmin - t1) / dt;
		if (t1 + nt * dt < tmax) nt++;
		if (nt < 1) Melder_throw ("Duration is too short.");
		...
		return thee.transfer();
	} catch (MelderError) { rethrowmzero ("Pitch not created from PitchTier."); }
}
