		...
		while (! eof) {
			...
			if (numberOfBytesRead == 0)
				...
				/* 
					...
					data is 16 bit
				*/
				const integer numberOfValuesRead = numberOfBytesRead / wordSizeInBytes;
				Melder_require (currentSample + numberOfValuesRead <= numberOfSamples * numberOfChannels,
					U"The number of samples read is too large.");
				for (integer ivalue = 1; ivalue <= numberOfValuesRead; ivalue ++) {
					currentSample ++; 
					const integer isample = (currentSample - 1) / numberOfChannels + 1;
					const integer ichannel = (currentSample - 1) % numberOfChannels + 1;
	...
	try {
		...
		*/
		...
		while (true) { // we repeat if the bitstream is chained
			...
			*/
			readBuffer = ogg_sync_buffer (& oy, bufferSize);
			bytesRead = fread (readBuffer, 1, bufferSize, f);
			ogg_sync_wrote (& oy, bytesRead);

			/* Get the first page. */
			if (ogg_sync_pageout (& oy, & og) != 1) {
				if (bytesRead < bufferSize) // have we simply run out of data?  If so, we're done.
					break;
			...
			*/
			...
			Melder_assert (numberOfChannels == vi.channels);
			/* 
