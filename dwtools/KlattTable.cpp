...
typedef struct structKlattFrame
{
	...
	long Fhz[9];	/* Formant freqs in Hz */
	long Bhz[9];	/* bandwidths in Hz */
	long Bphz[7];	/* Bandwidths, parallel branch */
...
typedef struct structKlattGlobal
{
	...
	long num_samples; /* number of glottal samples */
...
KlattTable KlattTable_readFromRawTextFile (MelderFile fs)
{
	try {
		...
		if (thy nx != KlattTable_NPAR) Melder_throw ("A KlattTable needs ",  KlattTable_NPAR, " columns.");
		...
		return me.transfer();
	} catch (MelderError) { Melder_throw ("KlattTable not read from file."); }
}
...
static KlattGlobal KlattGlobal_create (double samplingFrequency)
{
	...
	try {
		...
		return me;
	} catch (MelderError) { KlattGlobal_free (me); Melder_throw ("KlattGlobal not created."); }
}

static void KlattGlobal_init (KlattGlobal me, int synthesisModel, int numberOfFormants, int glottalSource,
	double frameDuration, long flutter, int outputType)
{
...
KlattTable KlattTable_create (double frameDuration, double totalDuration)
{
	try {
		...
		return me.transfer();
	} catch (MelderError) { Melder_throw ("KlattTable not created."); }
}
...
static void KlattGlobal_getFrame (KlattGlobal me, KlattFrame thee)
{
	...
	for (long i = 8; i > 0; i--)
	{
		if (my nfcascade >= i) Filter_setFB (my rc[i], thy Fhz[i], thy Bhz[i]);
	}
...
static float KlattGlobal_sampled_source (KlattGlobal me)
{
	...
	if(my T0 != 0)
	{
		...
		long current_value = my natural_samples[itemp];
		long  next_value = my natural_samples[itemp+1];
...
static void KlattGlobal_pitch_synch_par_reset (KlattGlobal me)
{
	...
	if (my F0hz10 > 0)
	{
		...
		if (my nopen < 40)
		{
			/* F0 max = 1000 Hz */
			my nopen = 40;
			Melder_warning2 (L"Warning: minimum glottal open period is 10 samples.\n"
...
{
	double out, frics, glotout, aspiration, par_glotout, noise, sourc, voice;
...
Sound KlattTable_to_Sound (KlattTable me, double samplingFrequency, int synthesisModel, int numberOfFormants, double frameDuration, int glottalSource, double flutter, int outputType)
{
	...
	try {
		...
		KlattGlobal thee = KlattGlobal_create (samplingFrequency);
		KlattFrame frame = KlattFrame_create ();
		...
		return him.transfer ();
	} catch (MelderError) { KlattGlobal_free (thee); KlattFrame_free (frame); Melder_throw (me, ": no Sound created."); }
}
...
KlattTable KlattTable_createExample (void)
{
	...
	try {
		...
		return me.transfer();
	} catch (MelderError) { Melder_throw (" KlattTable example not created."); }
}
