...
Discriminant Pattern_and_Categories_to_Discriminant (Pattern me, Categories thee)
{
	try {
		...
		return him.transfer();
	} catch (MelderError) { rethrowmzero ("Discriminant not created from Pattern & Categories."); }
}
	...
	int poolCovarianceMatrices,int useAprioriProbabilities)
{
	try {
		...
		return him.transfer();
	} catch (MelderError) { rethrowmzero ("Categories not created from Pattern & Discriminant."); }
}
