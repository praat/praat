...
/*
	Workhorse for calculation of:
		cross-correlation: correlate == true
		auto-correlation:  correlate == true, me == thee
		convolution:       correlate == false
	Cross-correlation (f,g) = int
*/
static Sound _Sounds_convolve (Sound me, Sound thee, bool correlate, bool normalize)
{
	long i;
	double df = my xmax - my xmin, dg = thy xmax - thy xmin, dhim = df + dg;
	Spectrum gs = NULL, fs = NULL;
	Sound him = NULL;

	if (my dx != thy dx) return Melder_errorp ("(Sounds_crossCorrelation:) Sampling frequencies do not match.");
	if (my ny != 1 || thy ny != 1) return Melder_errorp ("(Sounds_crossCorrelation:) Number of channels should equal one.");

	Sound buf = Sound_createSimple (1, dhim, 1 / my dx);
	if (buf == NULL) return NULL;

	for (i = 1; i <= my nx; i++) { buf -> z[1][i] = my z[1][i]; }
	fs = Sound_to_Spectrum (buf, 0);
	if (fs == NULL) goto end;

	if (me == thee)
	{
		gs = fs; // No need to calculate spectrum again.
	}
	else
	{
		// Reuse buf
		for (i = 1; i <= thy nx; i++) { buf -> z[1][i] = thy z[1][i]; }
		for (i = thy nx + 1; i <= my nx; i++) { buf -> z[1][i] = 0;	}

		gs = Sound_to_Spectrum (buf, 0);
		if (gs == NULL) goto end;
	}
	// Cross-corr:  conj(fs) . (gs) => (a-ib) . (c+id)
	// Convolution: (fs) . (gs) => (a+ib) . (c+id)
	// (a+ib) . (c+id) = (ac-bd) + i(ad + bc))
	for (i = 1; i <= gs -> nx; i++)
	{
		double a = fs -> z[1][i];
		double b = correlate ? - fs -> z[2][i] : fs -> z[2][i];
		double c = gs -> z[1][i];
		double d = gs -> z[2][i];
		gs -> z[1][i] = a * c - b * d;
		gs -> z[2][i] = a * d + b * c;
	...
	him = Spectrum_to_Sound (gs); // Pretend him is a sound!

	if (correlate) // Fold first part to back
	{
		for (i = 1; i <= thy nx; i++) { buf -> z[1][i] = his z[1][i]; }
		for (i = thy nx + 1; i <= his nx; i++) { his z[1][i - thy nx] = his z[1][i]; }
		for (i = 1; i <= thy nx; i++) { his z[1][his nx - thy nx + i] = buf -> z[1][i]; }
	}

	if (normalize)
	{
		double normf = NUMdvector_getNorm2 (my z[1], my nx);
		double normg = me == thee ? normf : NUMdvector_getNorm2 (thy z[1], thy nx);
		if (normf != 0 && normg != 0)
		{
			double factor = 1 / (my dx * normf * normg); // 1 / (sqrt(sum2f*dx)sqrt(sum2g*dx))
			for (i = 1; i <= his nx; i++) { his z[1][i] *= factor; }
		}
	}
...
{
	Sound him = _Sounds_convolve (me, thee, true, normalize);
	if (him == NULL) return NULL;
	Function_shiftXTo (him, his xmin, -(my xmax - my xmin));
	return him;
}

Sound Sound_autoCorrelate (Sound me, bool normalize)
{
	return Sounds_crossCorrelate_long (me, me, normalize);
}
