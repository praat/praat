...
void LongSounds_appendToExistingSoundFile (Collection me, MelderFile file)
{
	...
	try {
		if (my size < 1) Melder_throw ("No Sound or LongSound objects to append.");
		...
		*/
		...
		for (long i = 1; i <= my size; i++)
		{
			...
			if (! sampleRatesMatch) Melder_throw ("Sample rates do not match.");
			if (! numbersOfChannelsMatch) Melder_throw ("Cannot mix stereo and mono.");
		}
