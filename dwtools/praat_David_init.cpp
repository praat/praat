...
DIRECT (Categories_getNumberOfCategories)
	LOOP {
		iam(Categories);
		Melder_information2 (Melder_integer (my size), L" categories");
	}
END
...
DIRECT (CCA_getNumberOfCorrelations)
	LOOP {
		iam (CCA);
		Melder_information1 (Melder_double (my numberOfCoefficients));
	}
END
...
DO
	LOOP {
		iam (CCA);
		Melder_information1 (Melder_double (CCA_getCorrelationCoefficient (me, GET_INTEGER (L"Coefficient number"))));
	}
END
...
DO
	LOOP {
		iam (TableOfReal);
		Melder_information1 (Melder_double (TableOfReal_getColumnSumByLabel (me, GET_STRING (L"Response"))));
	}
END
...
DO
	LOOP {
		iam (TableOfReal);
		Melder_information1 (Melder_double (TableOfReal_getRowSumByLabel (me, GET_STRING (L"Stimulus"))));
	}
END
...
DO
	LOOP {
		...
		Melder_information5 (Melder_double (p), L" (=probability, based on t = ", Melder_double (t), L" and ndf = ", Melder_integer (ndf));
	}
END
...
DO
	LOOP {
		iam (Covariance);
		Melder_information1 (Melder_double (SSCP_getFractionVariation (me, GET_INTEGER (L"From dimension"), GET_INTEGER (L"To dimension"))));
	}
END
...
DO
	...
	LOOP {
		iam (Discriminant);
		Melder_information1 (Melder_double (Discriminant_getWilksLambda (me, from)));
	}
END
...
DO
	LOOP {
		...
		long group = Discriminant_groupLabelToIndex (me, GET_STRING (L"Group label"));
		REQUIRE (group > 0, L"Group label does not exist.")
		Melder_information1 (Melder_double (Discriminant_getConcentrationEllipseArea(me, group,
...
DO
	LOOP {
		...
		long group = Discriminant_groupLabelToIndex (me, GET_STRING (L"Group label"));
		REQUIRE (group > 0, L"Group label does not exist.")
		Melder_information1 (Melder_double (Discriminant_getLnDeterminant_group (me, group)));
	}
END
...
DIRECT (Discriminant_getLnDeterminant_total)
	LOOP {
		iam (Discriminant);
		Melder_information1 (Melder_double (Discriminant_getLnDeterminant_total (me)));
	}
END
...
DIRECT (Discriminant_getNumberOfFunctions)
	LOOP {
		iam (Discriminant);
		Melder_information1 (Melder_integer (Discriminant_getNumberOfFunctions (me)));
	}
END
...
DIRECT (Discriminant_getDimensionOfFunctions)
	LOOP {
		iam (Discriminant);
		Melder_information1 (Melder_integer (Eigen_getDimensionOfComponents (me)));
	}
END
...
DIRECT (Discriminant_getNumberOfGroups)
	LOOP {
		iam (Discriminant);
		Melder_information1 (Melder_integer (Discriminant_getNumberOfGroups (me)));
	}
END
...
DO
	LOOP {
		iam (Discriminant);
		Melder_information1 (Melder_integer (Discriminant_getNumberOfObservations (me, GET_INTEGER (L"Group"))));
	}
END
...
DO
	LOOP {
		iam (DTW);
		Melder_information1 (Melder_double (DTW_getPathY (me, GET_REAL (L"Time"))));
	}
END
...
DIRECT (DTW_distancesToMatrix)
	LOOP {
		iam (DTW);
		praat_new (DTW_distancesToMatrix (me), 0);
	}
END
...
DO
	LOOP {
		...
		if (number > my numberOfEigenvalues) Melder_throw ("Eigenvalue number must be smaller than ", my numberOfEigenvalues + 1);
...
DO
	LOOP {
		iam (Eigen);
		Melder_information1 (Melder_double (Eigen_getSumOfEigenvalues(me, GET_INTEGER (L"left Eigenvalue range"), GET_INTEGER (L"right Eigenvalue range"))));
	}
END
...
DO
	LOOP {
		iam (Eigen);
		Melder_information1 (Melder_double (Eigen_getEigenvectorElement (me, GET_INTEGER (L"Eigenvector number"), GET_INTEGER (L"Element number"))));
	}
END
...
DO
	...
	praat_new (Eigen_and_Matrix_project (me, mat, GET_INTEGER (L"Number of dimensions")), 0);
END

DIRECT (Eigen_and_SSCP_project)
	Eigen me = FIRST_GENERIC (Eigen);
	SSCP cp = FIRST (SSCP);
	praat_new (Eigen_and_SSCP_project (me, cp), 0);
END

DIRECT (Eigen_and_Covariance_project)
	Eigen me = FIRST_GENERIC (Eigen);
	Covariance cv = FIRST (Covariance);
	praat_new (Eigen_and_Covariance_project (me, cv), 0);
END
...
DIRECT (Index_getNumberOfClasses)
	LOOP {
		iam (Index);
		Melder_information1 (Melder_integer (my classes -> size));
	}
END
...
DO
	...
	LOOP {
		...
		if (klas > numberOfClasses) Melder_throw ("Index must be less than or equal ", numberOfClasses, L".");
...
DO
	...
	LOOP {
		iam (StringsIndex);
		if (index > my numberOfElements) Melder_throw ("Index must be less than or equal ", my numberOfElements, L".");
...
DO
	...
	LOOP {
		iam (Index);
		if (index > my numberOfElements) Melder_throw ("Index must be less than or equal ", my numberOfElements, L".");
		Melder_information1 (Melder_integer (my classIndex[index]));
	}
END
...
DO
	LOOP {
		iam (Index);
		praat_new (Index_to_Permutation_permuteRandomly (me, GET_INTEGER (L"Permute within classes")), 0);
	}
END
...
DIRECT (StringsIndex_to_Strings)
	LOOP {
		iam (StringsIndex);
		praat_new (StringsIndex_to_Strings (me), 0);
	}
END
...
DO
	LOOP {
		iam (Excitations);
		praat_new (Excitations_getItem (me, GET_INTEGER (L"Item number")), 0);
	}
END
...
DO
	LOOP {
		iam (Excitations);
		praat_new (Excitations_to_Pattern (me, GET_INTEGER (L"Join")), 0);
	}
END
...
DIRECT (Excitations_to_TableOfReal)
	LOOP {
		iam (Excitations);
		praat_new (Excitations_to_TableOfReal (me), 0);
	}
END
...
DIRECT (FilterBank_to_Matrix)
	LOOP {
		iam (FilterBank);
		praat_new (FilterBank_to_Matrix (me), 0);
	}
END
...
DIRECT (FilterBank_to_Intensity)
	LOOP {
		iam (FilterBank);
		praat_new (FilterBank_to_Intensity (me), 0);
	}
END
...
DO
	LOOP {
		iam (FunctionTerms);
		Melder_information1 (Melder_double (FunctionTerms_evaluate (me, GET_REAL (L"X"))));
	}
END
...
DIRECT (FunctionTerms_getNumberOfCoefficients)
	LOOP {
		iam (FunctionTerms);
		Melder_information1 (Melder_integer (my numberOfCoefficients));
	}
END
...
DO
	...
	LOOP {
		iam (FunctionTerms);
		if (index > my numberOfCoefficients) Melder_throw ("Index too large.");
		Melder_information1 (Melder_double (my coefficients[index]));
	}
END
...
DIRECT (FunctionTerms_getDegree)
	LOOP {
		iam (FunctionTerms);
		Melder_information1 (Melder_integer (FunctionTerms_getDegree (me)));
	}
END
...
DO
	...
	LOOP {
		iam (FunctionTerms);
		if (xmax <= xmin) Melder_throw ("Xmax must be larger than Xmin.");
...
DO
	LOOP {
		...
		praat_new (Intensity_to_TextGrid_detectSilences (me, GET_REAL (L"Silence threshold"),
			...
			GET_STRING (L"Silent interval label"), GET_STRING (L"Sounding interval label")), 0);
	}
END
...
DO
	...
	if (xmax <= xmin) Melder_throw ("Xmin must be smaller than Xmax.");
...
DO
	...
	if (flutter < 0 || flutter > 100) Melder_throw ("Flutter must be between 0 and 100%.");
	LOOP {
		iam (KlattTable);
		praat_new (KlattTable_to_Sound (me, GET_REAL (L"Sampling frequency"), GET_INTEGER (L"Synthesis model"),
			...
			GET_REAL (L"Flutter percentage"), outputType), 0);
	}
END
...
DO
	LOOP {
		iam (KlattTable);
		praat_new (KlattTable_to_KlattGrid (me, GET_REAL (L"Frame duration")), 0);
	}
END
...
DIRECT (KlattTable_to_Table)
	LOOP {
		iam (KlattTable);
		praat_new (KlattTable_to_Table (me), 0);
	}
END
...
DIRECT (Table_to_KlattTable)
	LOOP {
		iam (Table);
		praat_new (Table_to_KlattTable (me), 0);
	}
END
...
DIRECT (LegendreSeries_to_Polynomial)
	LOOP {
		iam (LegendreSeries);
		praat_new (LegendreSeries_to_Polynomial (me), 0);
	}
END
/********************* LongSound **************************************/
...
DO
	...
	if (scale < 1 || scale > 4) Melder_throw ("Scale must be in (0,4) interval.");
	autoPraatPicture picture;
	LOOP {
		iam (Matrix);
...
DIRECT (Matrix_Categories_to_TableOfReal)
	...
	praat_new (Matrix_and_Categories_to_TableOfReal (me, cat), 0);
END
...
DIRECT (Matrix_to_Activation)
	LOOP {
		iam (Matrix);
		praat_new (Matrix_to_Activation (me), 0);
	}
END
...
DO
	...
	Melder_assert (m1 && m2);
	praat_new (Matrices_to_DTW (m1, m2, begin, end, slope, GET_REAL (L"Distance metric")), 0);
...
DO
	LOOP {
		iam (Matrix);
		praat_new (Matrix_to_Pattern (me, GET_INTEGER (L"Join")), 0);
	}
END
...
DIRECT (Matrixft_getNumberOfFrequencies)
	LOOP {
		iam (Matrix);
		Melder_information1 (Melder_double (my ny));
	}
END
...
DO
	LOOP {
		iam (Matrix);
		Melder_information1 (Melder_double (Matrix_rowToY (me, GET_INTEGER (L"Row number"))));
	}
END
...
DO
	LOOP {
		iam (Matrix);
		Melder_information1 (Melder_double (Matrix_columnToX (me, GET_INTEGER (L"Column number"))));
	}
END
...
DO
	...
	LOOP {
		iam (Matrix);
		if (f < my ymin || f > my ymax) Melder_throw ("Frequency out of range.");
		if (t < my xmin || t > my xmax) Melder_throw ("Time out of range.");
...
DO
	LOOP {
		iam (MelFilter);
		praat_new (MelFilter_to_MFCC (me, GET_INTEGER (L"Number of coefficients")), 0);
	}
END
...
DO
	LOOP {
		...
		praat_new (MFCC_to_MelFilter (me, GET_INTEGER (L"From coefficient"),
			...
			GET_REAL (L"Distance between filters")), 0);
	}
END
...
DIRECT (Pattern_to_Matrix)
	LOOP {
		iam (Pattern);
		praat_new (Pattern_to_Matrix (me), 0);
	}
END
...
DO
	...
	Melder_information1 (Melder_double (PCA_and_TableOfReal_getFractionVariance (me, tab,
...
DIRECT (PCA_and_Configuration_to_TableOfReal_reconstruct)
	...
	praat_new (PCA_and_Configuration_to_TableOfReal_reconstruct (me, conf), 0);
END
...
DO
	long dimension = GET_INTEGER (L"Number of dimensions");
	if (dimension < 0) Melder_throw ("Number of dimensions must be greater equal zero.");
	PCA me = FIRST (PCA);
	TableOfReal tab = FIRST_GENERIC (TableOfReal);
	praat_new (PCA_and_TableOfReal_to_Configuration (me, tab, dimension), 0);
END
...
DO
	...
	LOOP {
		iam (PCA);
		if (number > my dimension) Melder_throw ("Number may not be larger than ", my dimension, ".");
		Melder_information4 (Melder_double(my centroid[number]), L" (element ", Melder_integer(number), L")");
	}
END
...
DO
	...
	LOOP {
		iam (Eigen);
		if (f <= 0 || f > 1) Melder_throw ("The variance fraction must be in interval (0-1).");
		Melder_information1 (Melder_integer (Eigen_getDimensionOfFraction (me, f)));
	}
END
...
DO
	...
	if (from > to) Melder_throw ("The second component must be greater than or equal to the first component.");
	LOOP {
		iam (Eigen);
		if (from > to) Melder_throw ("The second component must be greater than or equal to the first component.");
		Melder_information1 (Melder_double (Eigen_getCumulativeContributionOfComponents (me, from, to)));
	}
END
...
DO
	LOOP {
		iam (PCA);
		praat_new (PCA_to_TableOfReal_reconstruct1 (me, GET_STRING (L"Coefficients")), 0);
	}
END
...
DIRECT (PCAs_getAngleBetweenPc1Pc2Plane_degrees)
	...
	Melder_assert (p1 && p2);
	Melder_information2 (Melder_double (Eigens_getAngleBetweenEigenplanes_degrees (p1, p2)),
...
DO
	...
	praat_new (p, GET_STRING (L"Name"), 0);
END

DIRECT (Permutation_getNumberOfElements)
	LOOP {
		iam (Permutation);
		Melder_information1 (Melder_integer (my numberOfElements));
	}
END
...
DO
	...
	LOOP {
		iam (Permutation);
		Melder_information4 (Melder_integer (Permutation_getValueAtIndex (me, index)), L" (value, at index = ",
...
DO
	...
	LOOP {
		iam (Permutation);
		Melder_information4 (Melder_integer (Permutation_getIndexAtValue (me, value)), L" (index, at value = ",
...
DO
	...
	LOOP {
		iam (Permutation);
		praat_new (Permutation_rotate (me, GET_INTEGER (L"left Index range"), GET_INTEGER (L"right Index range"), step), Thing_getName (me), L"_rot", Melder_integer (step));
	}
END
...
DO
	LOOP {
		...
		GET_REAL (L"Pitch floor"),GET_REAL (L"Pitch ceiling")), 0);
	}
END
...
DO
	double xmin = GET_REAL (L"Xmin"), xmax = GET_REAL (L"Xmax");
	if (xmin >= xmax) Melder_throw ("Xmin must be smaller than Xmax.");
	praat_new (Polynomial_createFromString (xmin, xmax,
		GET_STRING (L"Coefficients")), GET_STRING (L"Name"));
END
...
DIRECT (Polynomial_getDerivative)
	LOOP {
		iam (Polynomial);
		praat_new (Polynomial_getDerivative (me), 0);
	}
END
...
DIRECT (Polynomial_getPrimitive)
	LOOP {
		iam (Polynomial);
		praat_new (Polynomial_getPrimitive (me), 0);
	}
END
...
DO
	...
	LOOP {
		iam (Polynomial);
		praat_new (Polynomial_scaleX (me, xmin, xmax), 0);
	}
END
...
DIRECT (Polynomial_to_Roots)
	LOOP {
		iam (Polynomial);
		praat_new (Polynomial_to_Roots (me), 0);
	}
END
...
DO
	...
	LOOP {
		iam (Polynomial);
		praat_new (Polynomial_to_Spectrum (me, GET_REAL (L"Nyquist frequency"), n, 1.0), 0);
	}
END
...
DO
	...
	*/
	...
	if (wantq) praat_new (aq.transfer(), Thing_getName (p1), L"_q");
	if (wantr) praat_new (ar.transfer(), Thing_getName (p1), L"_r");
END
...
DIRECT (Roots_getNumberOfRoots)
	LOOP {
		iam (Roots);
		Melder_information1 (Melder_integer (Roots_getNumberOfRoots (me)));
	}
END
...
DO
	...
	LOOP {
		iam (Roots);
		praat_new (Roots_to_Spectrum (me, GET_REAL (L"Nyquist frequency"), n, 1.0), 0);
	}
END
...
DO
	...
	praat_new (Sound_and_Pitch_to_FormantFilter (me, p, GET_REAL (L"Analysis window duration"),
		...
		GET_REAL (L"Relative bandwidth")), 0);
END
...
DO
	...
	praat_new (Sound_and_Pitch_changeGender_old (me, p, GET_REAL (L"Formant shift ratio"),
		GET_REAL (L"New pitch median"), GET_REAL (L"Pitch range factor"), GET_REAL (L"Duration factor")), 0);
END
...
DO
	...
	praat_new (Sound_and_Pitch_changeSpeaker (me, p, GET_REAL (L"Multiply formants by"),
		GET_REAL (L"Multiply pitch by"), GET_REAL (L"Multiply pitch range by"), GET_REAL (L"Multiply duration")), 0);
END
...
DO
	...
	if (gamma < 0) Melder_throw ("Gamma cannot be negative.\nPlease use a positive or zero gamma.");
	if (bandwidth < 0) Melder_throw ("Bandwidth cannot be negative.\nPlease use a positive or zero bandwidth.");
...
DO
	...
	Sound_create_checkCommonFields (dia, &startingTime, &finishingTime, &samplingFrequency);
	if (amplitudeRange < 0) Melder_throw ("Amplitude range cannot be negative.\nPlease use a positive or zero amplitude range.");
...
DO
	LOOP {
		...
		praat_new (Sound_to_TextGrid_detectSilences (me, GET_REAL (L"Minimum pitch"), GET_REAL (L"Time step"),
			...
			GET_STRING (L"Sounding interval label")), 0);
	}
END
...
DO
	...
	if (minimumPitch >= ceiling) Melder_throw ("Minimum pitch should be smaller than ceiling.");
	if (ceiling >= fmax) Melder_throw ("Maximum frequency must be greater than or equal to ceiling.");
	LOOP {
		iam (Sound);
		praat_new (Sound_to_Pitch_shs (me, GET_REAL (L"Time step"), minimumPitch, fmax, ceiling,
			...
			GET_REAL (L"Compression factor"), GET_INTEGER (L"Number of points per octave")), 0);
	}
END
...
DO
	LOOP {
		...
		praat_new (Sound_to_KlattGrid_simple (me, GET_REAL (L"Time step"),
			...
			GET_REAL (L"Minimum pitch"), GET_INTEGER (L"Subtract mean")), 0);
	}
END
...
DO
	...
	if (fmax <= fmin) Melder_throw ("Maximum frequency must be larger than minimum frequency.");
	LOOP {
		iam (Sound);
		...
		GET_REAL (L"Ceiling"), GET_INTEGER (L"Max. number of candidates")), 0);
	}
END
...
DO
	LOOP {
		iam (Sound);
		praat_new (Sound_filterByGammaToneFilter4 (me, GET_REAL (L"Centre frequency"), GET_REAL (L"Bandwidth")), 0);
	}
END
...
DO
	...
	LOOP {
		...
		praat_new (Sound_changeSpeaker (me, minimumPitch, maximumPitch,
			...
			GET_REAL (L"Multiply pitch range by"), GET_REAL (L"Multiply duration by")), 0);
	}
END
...
DO
	...
	Melder_assert (s1 && s2);
	praat_new (Spectrograms_to_DTW (s1, s2, begin, end, slope, 1), 0);
END
...
DIRECT (Spline_getOrder)
	LOOP {
		iam (Spline);
		Melder_information1 (Melder_integer (Spline_getOrder (me)));
	}
END
...
DO
	...
	LOOP {
		iam (Spline);
		praat_new (Spline_scaleX (me, xmin, xmax), 0);
	}
END
...
DO
	...
	LOOP {
		iam (SSCP);
		Melder_information1 (Melder_double (SSCP_getConcentrationEllipseArea (me, conf, 1, d1, d2)));
	}
END
...
DO
	LOOP {
		iam (SSCP);
		Melder_information1 (Melder_double (SSCP_getFractionVariation (me,
...
DO
	...
	LOOP {
		iam (SSCP);
		Melder_information1 (Melder_double (SSCP_getConcentrationEllipseArea (me, nsigmas, 0, d1, d2)));
	}
END
...
DIRECT (SSCP_getDegreesOfFreedom)
	LOOP {
		iam (SSCP);
		Melder_information1 (Melder_double (SSCP_getDegreesOfFreedom (me)));
	}
END
...
DIRECT (SSCP_getTotalVariance)
	LOOP {
		iam (SSCP);
		Melder_information1 (Melder_double (SSCP_getTotalVariance (me)));
	}
END
...
DO
	...
	LOOP {
		iam (SSCP);
		if (number < 1 || number > my numberOfColumns) Melder_throw ("\"Number\" must be smaller than ", my numberOfColumns + 1, ".");
...
DIRECT (SSCP_getLnDeterminant)
	LOOP {
		iam (SSCP);
		Melder_information1 (Melder_double (SSCP_getLnDeterminant (me)));
	}
END
...
DO
	LOOP {
		iam (Strings);
		praat_new (Strings_extractPart (me, GET_INTEGER (L"From index"), GET_INTEGER (L"To index")), 0);
	}
END
...
DO
	LOOP {
		iam (Strings);
		praat_new (Strings_to_Permutation (me, GET_INTEGER (L"Sort")), 0);
	}
END
...
DIRECT (Strings_and_Permutation_permuteStrings)
	...
	praat_new (Strings_and_Permutation_permuteStrings (me, p), 0);
END
...
DO
	LOOP {
		iam (SVD);
		praat_new (SVD_to_TableOfReal (me, GET_INTEGER (L"First component"), GET_INTEGER (L"Last component")), 0);
	}
END
...
DIRECT (TableOfReal_and_Permutation_permuteRows)
	...
	praat_new (TableOfReal_and_Permutation_permuteRows (me, p),
		Thing_getName (me), L"_", Thing_getName (p));
END

DIRECT (TableOfReal_to_Permutation_sortRowlabels)
	LOOP {
		iam (TableOfReal);
		praat_new (TableOfReal_to_Permutation_sortRowLabels (me), 0);
	}
END
...
DO
	...
	LOOP {
		iam (TableOfReal);
		praat_new (TableOfReal_to_Configuration_lda (me, dimension), 0);
	}
END
...
DO
	LOOP {
		iam (TableOfReal);
		praat_new (TableOfReal_to_CCA (me, GET_INTEGER (L"Dimension of dependent variate")), 0);
	}
END
...
DO
	LOOP {
		iam (TableOfReal);
		praat_new (TableOfReal_to_Configuration_pca (me, GET_INTEGER (L"Number of dimensions")), 0);
	}
END
...
DIRECT (TableOfReal_to_Discriminant)
	LOOP {
		iam (TableOfReal);
		praat_new (TableOfReal_to_Discriminant (me), 0);
	}
END
...
DIRECT (TableOfReal_to_PCA)
	LOOP {
		iam (TableOfReal);
		praat_new (TableOfReal_to_PCA (me), 0);
	}
END
...
DO
	LOOP {
		...
		praat_new (TableOfReal_to_SSCP (me, GET_INTEGER (L"Begin row"), GET_INTEGER (L"End row"),
			GET_INTEGER (L"Begin column"), GET_INTEGER (L"End column")), 0);
	}
END
...
DO
	LOOP {
		iam (TableOfReal);
		Melder_information1 (Melder_double (TableOfReal_getColumnSum (me, GET_INTEGER (L"Column"))));
	}
END
...
DO
	LOOP {
		iam (TableOfReal);
		Melder_information1 (Melder_double (TableOfReal_getRowSum (me, GET_INTEGER (L"Row"))));
	}
END
...
DIRECT (TableOfReal_getGrandSum)
	LOOP {
		iam (TableOfReal);
		Melder_information1 (Melder_double (TableOfReal_getGrandSum (me)));
	}
END
