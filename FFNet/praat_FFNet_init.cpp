...
DO
	LOOP {
		...
		if (unit > c -> size) Melder_throw (L"Output unit cannot be larger than ", c -> size, L".");
...
DIRECT (FFNet_getNumberOfOutputWeights)
	LOOP {
		iam(FFNet);
		Melder_information2 (Melder_integer (my nUnitsInLayer[my nLayers] * (my nUnitsInLayer[my nLayers - 1]+1)),
			L" weights");
	}
END
...
DIRECT (FFNet_getMinimum)
	LOOP {
		iam(FFNet);
		Melder_information1 (Melder_double (FFNet_getMinimum (me)));
	}
END
...
DO
	...
	praat_new (FFNet_Activation_to_Categories (me, thee, GET_INTEGER (L"Kind of labeling")), Thing_getName (me), L"_", Thing_getName (thee));
END
...
DO
	FFNet me = FIRST (FFNet);
	Pattern thee = FIRST (Pattern);
	praat_new (FFNet_Pattern_to_Activation (me, thee, GET_INTEGER (L"Layer")), Thing_getName (me), L"_", Thing_getName(thee));
END
...
DO
	FFNet me = FIRST (FFNet);
	Pattern thee = FIRST (Pattern);
	praat_new (FFNet_Pattern_to_Categories (me, thee, GET_INTEGER (L"Determine output category as")), Thing_getName (me), L"_", Thing_getName (thee));
END
...
DO
	...
	Melder_information1 (Melder_double (FFNet_Pattern_Activation_getCosts_total (me, thee, him, GET_INTEGER (L"Cost function"))));
END
...
DO
	...
	Pattern thee = FIRST (Pattern);
	Activation him = FIRST (Activation);
	Melder_information1 (Melder_double (FFNet_Pattern_Activation_getCosts_average (me, thee, him, GET_INTEGER (L"Cost function"))));
END
