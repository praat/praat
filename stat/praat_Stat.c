...
DO
	WHERE (SELECTED) {
		Table me = OBJECT;
		long icol = Table_columnLabelToIndex (me, GET_STRING (L"left Columns"));
		long jcol = Table_columnLabelToIndex (me, GET_STRING (L"right Columns"));
...
DO
	...
	WHERE (SELECTED) {
		Table me = OBJECT;
		long xcolumn = Table_columnLabelToIndex (me, GET_STRING (L"Horizontal column"));
		long ycolumn = Table_columnLabelToIndex (me, GET_STRING (L"Vertical column"));
...
DO
	...
	WHERE (SELECTED) {
		Table me = OBJECT;
		long icol = Table_columnLabelToIndex (me, GET_STRING (L"Extract all rows where column..."));
...
DO
	WHERE (SELECTED) {
		Table me = OBJECT;
		long icol = Table_columnLabelToIndex (me, GET_STRING (L"Column label"));
		if (icol == 0) return Melder_error1 (L"No such column.");
...
DO
	WHERE (SELECTED) {
		Table me = OBJECT;
		long icol1 = Table_columnLabelToIndex (me, GET_STRING (L"From column label"));
		if (icol1 == 0) return Melder_error1 (L"No such column.");
		long icol2 = Table_columnLabelToIndex (me, GET_STRING (L"To column label"));
...
FORM (Table_getColumnIndex, L"Table: Get column index", 0)
	...
	OK
DO
	Melder_information1 (Melder_integer (Table_columnLabelToIndex (ONLY_OBJECT, GET_STRING (L"Column label"))));
END
...
DO
	Table me = ONLY_OBJECT;
	long column = Table_columnLabelToIndex (me, GET_STRING (L"Column label"));
	if (column == 0) return Melder_error1 (L"No such column.");
	long groupColumn = Table_columnLabelToIndex (me, GET_STRING (L"Group column"));
...
DO
	...
	REQUIRE (icol > 0, L"No such column.")
	REQUIRE (icol <= my numberOfColumns, L"Column number must not be greater than number of columns.")
	Melder_information1 (Melder_double (Table_getQuantile (ONLY_OBJECT, icol, GET_REAL (L"Quantile"))));
END
...
DO
	Table me = ONLY_OBJECT;
	long column1 = Table_columnLabelToIndex (me, GET_STRING (L"left Columns"));
	long column2 = Table_columnLabelToIndex (me, GET_STRING (L"right Columns"));
...
DO
	Table me = ONLY_OBJECT;
	long column = Table_columnLabelToIndex (me, GET_STRING (L"Column"));
	if (column == 0) return Melder_error1 (L"No such column.");
	long groupColumn = Table_columnLabelToIndex (me, GET_STRING (L"Group column"));
	if (groupColumn == 0) return Melder_error1 (L"No such column.");
	double significanceLevel = GET_REAL (L"Significance level");
...
DO
	...
	WHERE (SELECTED) {
		...
		long xcolumn = Table_columnLabelToIndex (me, GET_STRING (L"Horizontal column"));
		long ycolumn = Table_columnLabelToIndex (me, GET_STRING (L"Vertical column"));
		long markColumn = Table_columnLabelToIndex (me, GET_STRING (L"Column with marks"));
...
DO
	WHERE (SELECTED) {
		Table_setColumnLabel (OBJECT, Table_columnLabelToIndex (OBJECT, GET_STRING (L"Old label")), GET_STRING (L"New label"));
		praat_dataChanged (OBJECT);
		iferror return 0;
	}
END
...
DO
	WHERE (SELECTED) {
		Table me = OBJECT;
		long icol = Table_columnLabelToIndex (me, GET_STRING (L"Column for row labels"));
...
void praat_uvafon_Stat_init (void) {
	...
	praat_addAction1 (classTable, 0, L"Query -                ", 0, 0, 0);
		...
		praat_addAction1 (classTable, 1, L"-- to regression --", 0, 1, 0);
		praat_addAction1 (classTable, 1, L"To linear regression", 0, 1, DO_Table_to_LinearRegression);
		praat_addAction1 (classTable, 1, L"To logistic regression", 0, 1, DO_Table_to_LogisticRegression);
