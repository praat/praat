/*------------------------------------------------------------------------*
 * static const char CVSID[] = "$Id: regularExp.c,v 1.25 2004/08/20 16:37:30 edg Exp $";
 ...
 *
 * djmw 20060620 change from version 1.12 to 1.25
 ...
 *
 * Paul Boersma 20070524: wchar_t version
 */
...
#if (LAST_PAREN > USHRT_MAX)
#error "Too many parentheses for storage in an unsigned short (LAST_PAREN too big.)"
#endif
...
#define GET_OP_CODE(p)  (*(wchar_t *)(p))
#define OPERAND(p)      ((p) + NODE_SIZE)
#define GET_OFFSET(p)   ((( *((p) + 1) & 0377) << 8) + (( *((p) + 2)) & 0377))
#define PUT_OFFSET_L(v) (wchar_t)(((v) >> 8) & 0377)
#define PUT_OFFSET_R(v) (wchar_t) ((v)       & 0377)
...
#define U_CHAR_AT(p)     ((unsigned int) *(wchar_t *)(p))
...
static wchar_t       *Reg_Parse;       /* Input scan ptr (scans user's regex) */
...
static wchar_t        Compute_Size;    /* Address of this used as flag. */
static wchar_t       *Code_Emit_Ptr;   /* When Code_Emit_Ptr is set to
...
static wchar_t      **Error_Ptr;       /* Place to store error messages so
                                          they can be returned by `CompileRE' */
static wchar_t        Error_Text [128];/* Sting to build error messages in. */

static wchar_t        White_Space [WHITE_SPACE_SIZE]; /* Arrays used by       */
static wchar_t        Word_Char   [ALNUM_CHAR_SIZE];  /* functions            */
static wchar_t        Letter_Char [ALNUM_CHAR_SIZE];  /* init_ansi_classes () */
                                                      /* and
                                                         shortcut_escape ().  */

static wchar_t        ASCII_Digits [] = L"0123456789"; /* Same for all */
                                                       /* locales.     */
static int            Is_Case_Insensitive;
static int            Match_Newline;

static int            Enable_Counting_Quantifier = 1;
static wchar_t        Brace_Char;
static wchar_t        Default_Meta_Char [] = L"{.*+?[(|)^<>$";
static wchar_t       *Meta_Char;

typedef struct { long lower; long upper; } len_range;

/* Forward declarations for functions used by `CompileREW'. */

static wchar_t       * alternative     (int *flag_param, len_range *range_param);
static wchar_t       * back_ref        (wchar_t *c, int *flag_param, int emit);
static wchar_t       * chunk           (int paren, int *flag_param, len_range *range_param);
static void            emit_byte       (wchar_t c);
static void            emit_class_byte (wchar_t c);
static wchar_t       * emit_node       (int op_code);
static wchar_t       * emit_special    (wchar_t op_code, unsigned long test_val, int index);
static wchar_t         literal_escape  (wchar_t c);
static wchar_t         numeric_escape  (wchar_t c, wchar_t **parse);
static wchar_t       * atom            (int *flag_param, len_range *range_param);
static void            reg_error       (wchar_t *str);
static wchar_t       * insert          (wchar_t op, wchar_t *opnd, long min, long max, int index);
static wchar_t       * next_ptr        (wchar_t *ptr);
static void            offset_tail     (wchar_t *ptr, int offset, wchar_t *val);
static void            branch_tail     (wchar_t *ptr, int offset, wchar_t *val);
static wchar_t       * piece           (int *flag_param, len_range *range_param);
static void            tail            (wchar_t *search_from, wchar_t *point_t);
static wchar_t       * shortcut_escape (wchar_t c, int *flag_param, int emit);
...
regexpW * CompileREW (const wchar_t *exp, wchar_t **errorText, int defaultFlags) {
	regexpW *comp_regex = NULL;
	wchar_t *scan;
	...
	*Error_Ptr = L"";
	...
	if (exp == NULL) REG_FAIL (L"NULL argument, `CompileREW\'");

	/* Initialize arrays used by function `shortcut_escape'. */

	if (! init_ansi_classes ()) REG_FAIL (L"internal error #1, `CompileREW\'");
	...
	for (pass = 1; pass <= 2; pass ++) {
		...
		if (pass == 1) {
			if (Reg_Size >= MAX_COMPILED_SIZE) {
				...
				swprintf  (Error_Text, 128, L"regexp > %lu bytes", MAX_COMPILED_SIZE);
				REG_FAIL (Error_Text);
			}

			/* Allocate memory. */

			comp_regex = (regexpW *) malloc (sizeof (regexpW) + Reg_Size * sizeof (wchar_t));

			if (comp_regex == NULL) REG_FAIL (L"out of memory in `CompileREW\'");

			Code_Emit_Ptr = (wchar_t *) comp_regex -> program;
		}
	}

	comp_regex -> program [1] = Total_Paren - 1;
	comp_regex -> program [2] = Num_Braces;
...
static wchar_t * chunk (int paren, int *flag_param, 
                              len_range *range_param) {

   register wchar_t       *ret_val = NULL;
   register wchar_t       *this_branch;
   register wchar_t       *ender = NULL;
   register          int   this_paren = 0;
		     ...
		     len_range range_local;
		     int   look_only = 0;
            wchar_t       *emit_look_behind_bounds = NULL;
   ...
   *flag_param = HAS_WIDTH;  /* Tentatively. */
   ...
   if (paren == PAREN) {
      if (Total_Paren >= NSUBEXP) {
         swprintf (Error_Text, 128, L"number of ()'s > %d", (int) NSUBEXP);
         REG_FAIL (Error_Text);
      }
...
static wchar_t * piece (int *flag_param, len_range *range_param) {
   ...
   if (!IS_QUANTIFIER (op_code)) {
      ...
      *range_param = range_local;
      ...
      for (i = 0; i < 2; i++) {
         ...
         while (isdigit (*Reg_Parse)) {
            ...
            if ((min_max [i] == 6553UL && (*Reg_Parse - '0') <= 5) ||
               ...
               if (i == 0) {
                  swprintf (Error_Text, 128, L"min operand of {%lu%c,???} > 65535",
                           min_max [0], *Reg_Parse);
               } else {
                  swprintf (Error_Text, 128, L"max operand of {%lu,%lu%c} > 65535",
      ...
      if (digit_present [0] && (min_max [0] == REG_ZERO) && !comma_present) {
         ...
         if (digit_present [0]) {
            swprintf (Error_Text, 128, L"{%lu,0} is an invalid range", min_max [0]);
            REG_FAIL (Error_Text);
         } else {
            REG_FAIL (L"{,0} is an invalid range");
         }
      ...
      if (*Reg_Parse != '}') {
         ...
         swprintf (Error_Text, 128, L"{%lu,%lu} is an invalid range",
                  min_max [0], min_max [1]);
         REG_FAIL (Error_Text);
      }
   }
   ...
   if (op_code == '{') {
	 ...
	 *range_param = range_local;
         ...
         swprintf (Error_Text, 128, L"number of {m,n} constructs > %d", UCHAR_MAX);
         REG_FAIL (Error_Text);
      }
   }
   ...
   if (!(flags_local & HAS_WIDTH)) {
      if (brace_present) {
         swprintf (Error_Text, 128, L"{%lu,%lu} operand could be empty",
                  min_max [0], min_max [1]);
      } else {
         swprintf (Error_Text, 128, L"%c operand could be empty", op_code);
      }
   ...
   *flag_param = (min_max [0] > REG_ZERO) ? (WORST | HAS_WIDTH) : WORST;
   ...
   if (IS_QUANTIFIER (*Reg_Parse)) {
      if (op_code == '{') {
         swprintf (Error_Text, 128, L"nested quantifiers, {m,n}%c", *Reg_Parse);
      } else {
         swprintf (Error_Text, 128, L"nested quantifiers, %c%c", op_code, *Reg_Parse);
      }
   ...
   *flag_param = WORST;  /* Tentatively. */
   ...
   switch (*Reg_Parse++) {
	 ...
	 *range_param = range_local;
      ...
      case '+':
      case '*':
         swprintf (Error_Text, 128, L"%c follows nothing", *(Reg_Parse - 1));
         REG_FAIL (Error_Text);
      ...
      case '[':
         {
            ...
            if (*Reg_Parse != ']') REG_FAIL (L"missing right \']\'");
      ...
      case '\\':
	    ...
	    break;
         ...
         if (wcslen (Error_Text) > 0) REG_FAIL (Error_Text);
      ...
      default:
                         ...
                         pointing at the back slash (\) character. */
         {
            ...
            for (; *Reg_Parse != '\0' &&
                   !wcschr (Meta_Char, (int) *Reg_Parse);
                 len++) {
            ...
            if (len <= 0) REG_FAIL (L"internal error #4, `atom\'");
...
static wchar_t * emit_special (
   wchar_t op_code,
   unsigned long test_val,
            int  index) {

   register wchar_t *ret_val = &Compute_Size;
   register wchar_t *ptr;
...
static void tail (wchar_t *search_from, wchar_t *point_to) {
...
static wchar_t * shortcut_escape (
   wchar_t  c,
   int           *flag_param,
   int            emit) {

   register wchar_t *class   = NULL;
   static   wchar_t *codes   = L"ByYdDlLsSwW";
            wchar_t *ret_val = (wchar_t *) 1; /* Assume success. */
            wchar_t *valid_codes;
...
static wchar_t numeric_escape (
   wchar_t    c,
   wchar_t  **parse) {

   static wchar_t digits [] = L"fedcbaFEDCBA9876543210";
   ...
   register wchar_t *scan;
   register wchar_t *pos_ptr;
   register wchar_t *digit_str;
   ...
   pos_ptr = (wchar_t *) wcschr ((wchar_t *) digit_str, (int) *scan);
   ...
   for (i = 0; pos_ptr != NULL && (i < width); i++) {
      ...
      scan++;
      pos_ptr = wcschr (digit_str, (int) *scan);
   }
   ...
   if (value == 0) {
      if (c == '0') {
         swprintf (Error_Text, 128, L"\\00 is an invalid octal escape");
      } else {
         swprintf (Error_Text, 128, L"\\%c0 is an invalid hexadecimal escape", c);
      }
   } else {
...
static wchar_t * back_ref (
   ...
   if (!is_cross_regex && !TEST_BIT (Closed_Parens, paren_no)) {
      swprintf (Error_Text, 128, L"\\%d is an illegal back reference", paren_no);
      return NULL;
   }
...
static wchar_t  *Reg_Input;           /* String-input pointer.         */
static wchar_t  *Start_Of_String;     /* Beginning of input, for ^     */
											/* and < checks.                 */
static wchar_t  *Look_Behind_To;      /* Position till were look behind
                                               can safely check back         */
static wchar_t **Start_Ptr_Ptr;       /* Pointer to `startp' array.    */
static wchar_t **End_Ptr_Ptr;         /* Ditto for `endp'.             */
static wchar_t  *Extent_Ptr_FW;       /* Forward extent pointer        */
static wchar_t  *Extent_Ptr_BW;       /* Backward extent pointer       */
static wchar_t  *Back_Ref_Start [10]; /* Back_Ref_Start [0] and        */
static wchar_t  *Back_Ref_End   [10]; /* Back_Ref_End [0] are not      */
...
static wchar_t  Default_Delimiters [256] = { 0 };

static wchar_t *Current_Delimiters;  /* Current delimiter table */

/* Forward declarations of functions used by `ExecRE' */

static int             attempt            (regexpW *, wchar_t *);
static int             match              (wchar_t *, int *);
static unsigned long   greedy             (wchar_t *, long);
static void            adjustcase         (wchar_t *, int, wchar_t);
static wchar_t       * makeDelimiterTable (wchar_t *, wchar_t *);

/*
 * ExecREW - match a `regexp' structure against a string
 *
...
int ExecREW (
   ...
   const wchar_t   *look_behind_to) {

   register wchar_t  *str;
            wchar_t **s_ptr;
            wchar_t **e_ptr;
                     int    ret_val = 0;
            wchar_t   tempDelimitTable [256];
                     int    i;

   s_ptr = (wchar_t **) prog->startp;
   e_ptr = (wchar_t **) prog->endp;
   ...
   Start_Of_String    = (wchar_t *) string;
   Look_Behind_To     = (wchar_t *) (look_behind_to?look_behind_to:string);

   Prev_Is_BOL        = ((prev_char == '\n') || (prev_char == '\0') ? 1 : 0);
   Succ_Is_EOL        = ((succ_char == '\n') || (succ_char == '\0') ? 1 : 0);
   Prev_Is_Delim      = (Current_Delimiters [(wchar_t)prev_char] ? 1 : 0);
   Succ_Is_Delim      = (Current_Delimiters [(wchar_t)succ_char] ? 1 : 0);
...
/*   Cross_Regex_Backref = cross_regex_backref; */
   ...
   if (Num_Braces > 0) {
      Brace =
         (brace_counts *) malloc (sizeof (brace_counts) + (size_t) Num_Braces);    // BUG (ppgb)
   ...
   for (i = 9; i > 0; i--) {
      *s_ptr++ = (wchar_t *) string;
      *e_ptr++ = (wchar_t *) string;
   }
   ...
   if (!reverse) { /* Forward Search */
      if (prog->anchor) {
         ...
         if (attempt (prog, (wchar_t *) string)) {
            ret_val = 1;
            goto SINGLE_RETURN;
         }

         for (str = (wchar_t *) string;
             *str != '\0' && str != (wchar_t *) end && !Recursion_Limit_Exceeded;
              str++) {
         ...
         if (!Recursion_Limit_Exceeded && !ret_val && *str == '\0' && str != (wchar_t *) end) {
            if (attempt (prog, str)) {
               ret_val = 1;
            }
         }
      ...
      if (prog->anchor) {
         ...
         for (str = (wchar_t *)(end - 1);
              str >= (wchar_t *) string && !Recursion_Limit_Exceeded;
              str--) {
         ...
         if (!Recursion_Limit_Exceeded && attempt (prog, (wchar_t *) string)) {
            ret_val = 1;
            goto SINGLE_RETURN;
         }
         ...
         for (str =  (wchar_t *) end;
            ...
            if (*str == (wchar_t)prog->match_start) {
               if (attempt (prog, str)) {
                  ret_val = 1;
                  break;
               }
   ...
   if (!initialized) {
      ...
      for (i = 1; i < (int)UCHAR_MAX; i++) {
         if (isalnum (i) || i == underscore) {
            Word_Char [word_count++] = (wchar_t) i;
         }

         if (isalpha (i)) {
            Letter_Char [letter_count++] = (wchar_t) i;
         }
         ...
         if (isspace (i) && (i != (int) '\n')) {
            White_Space [space_count++] = (wchar_t) i;
         }
...
static int attempt (regexpW *prog, wchar_t *string) {

   register          int    i;
   register wchar_t **s_ptr;
   register wchar_t **e_ptr;
   		     int    branch_index = 0; /* Must be set to zero ! */

   Reg_Input      = string;
   Start_Ptr_Ptr  = (wchar_t **) prog->startp;
   End_Ptr_Ptr    = (wchar_t **) prog->endp;
   s_ptr          = (wchar_t **) prog->startp;
   e_ptr          = (wchar_t **) prog->endp;
   ...
   if (match ((wchar_t *) (prog->program + REGEX_START_OFFSET),
	&branch_index)) {
      prog->startp [0] = (wchar_t *) string;
      prog->endp   [0] = (wchar_t *) Reg_Input;     /* <-- One char AFTER  */
      prog->extentpBW  = (wchar_t *) Extent_Ptr_BW; /*     matched string! */
      prog->extentpFW  = (wchar_t *) Extent_Ptr_FW;
...
static int match (wchar_t *prog, int *branch_index_param) {
      ...
      switch (GET_OP_CODE (scan)) {
         ...
         case NOT_BOUNDARY: /* \B (NOT a word boundary) */
		  ...
		  current_is_delim = Current_Delimiters [ *Reg_Input ];
	       ...
	       if (!(prev_is_delim ^ current_is_delim)) break;
	    }
         ...
         case ANY_OF:  /* [...] character class. */
            if (*Reg_Input == '\0') MATCH_RETURN (0); /* Needed because wcschr ()
                                                         considers \0 as a member
                                                         of the character set. */

            if (wcschr (OPERAND (scan), (int) *Reg_Input) == NULL) {
               MATCH_RETURN (0);
            }
         ...
         /* case X_REGEX_BR_CI: *** IMPLEMENT LATER */
            {
               register wchar_t *captured, *finish;
                                 int   paren_no;
...
static unsigned long greedy (wchar_t *p, long max) {

   register wchar_t *input_str;
   register wchar_t *operand;
   ...
   switch (GET_OP_CODE (p)) {
      ...
      case ANY_OF:  /* [...] character class. */
         while (count < max_cmp     &&
                *input_str != '\0'  &&
                wcschr (operand, (int) *input_str) != NULL) {
      ...
      case IS_DELIM: /* \y (a word delimiter char)
...
void SubstituteREW (
   ...
   register          int   len;
   register wchar_t  chgcase;

   if (prog == NULL || source == NULL || dest == NULL) {
      reg_error (L"NULL parm to `SubstituteREW\'");

      return;
   }

   if (U_CHAR_AT (prog->program) != MAGIC) {
      reg_error (L"damaged regexp passed to `SubstituteREW\'");
   ...
   while ((c = *src++) != '\0') {
      ...
      if (paren_no < 0) { /* Ordinary character. */
         ...
         (void) wcsncpy (dst, prog->startp [paren_no], len);
...
static void adjustcase (wchar_t *str, int len, wchar_t chgcase) {

   register wchar_t   *string = str;
            int        i;
...
static wchar_t * makeDelimiterTable (
   wchar_t *delimiters,
   wchar_t *table) {

   wchar_t *c;

   memset (table, 0, 256 * sizeof (wchar_t));

   for (c = (wchar_t *) delimiters; *c != '\0'; c++) {
      table [*c] = 1;
   }
...
void SetREDefaultWordDelimitersW (wchar_t *delimiters) {
