...
inline integer DigitstringNumber_compare (DigitstringNumber const& x, DigitstringNumber const& y) {
	return ( x.value < y.value ? -1 : x.value > y.value ? 1 : 
		(x.numberOfLeadingZeros > y.numberOfLeadingZeros ? - 1 : x.numberOfLeadingZeros < y.numberOfLeadingZeros ? 1 : 0));
}

inline void INTVECindex_inout (INTVEC index, constDigitstringNumberVEC const& v) {
	INTVECindex2_inout<constDigitstringNumberVEC, DigitstringNumber> (index, v, [](DigitstringNumber const& x, DigitstringNumber const& y) -> bool
		{ return ( x.value < y.value ? true : x.value > y.value ? false : 
		(x.numberOfLeadingZeros > y.numberOfLeadingZeros ? true : false) );}
	);
}
