...
void opus_tags_clear(OpusTags *_tags){
  ...
  for(ci=ncomments;ci-->0;)Melder_free(_tags->user_comments[ci]);
  Melder_free(_tags->user_comments);
  Melder_free(_tags->comment_lengths);
  Melder_free(_tags->vendor);
}
...
static int op_tags_ensure_capacity(OpusTags *_tags,size_t _ncomments){
  ...
  comment_lengths=(int *)Melder_realloc(_tags->comment_lengths,size);
  ...
  user_comments=(char **)Melder_realloc(_tags->user_comments,size);
...
static int opus_tags_parse_impl(OpusTags *_tags,
 const unsigned char *_data,size_t _len){
  ...
  if(len>0&&(_data[0]&1)){
    ...
    if(_tags!=NULL){
      _tags->user_comments[ncomments]=(char *)_Melder_malloc(len);
...
static int opus_tags_copy_impl(OpusTags *_dst,const OpusTags *_src){
  ...
  if(_src->comment_lengths!=NULL){
    ...
    if(len>0){
      _dst->user_comments[ncomments]=(char *)_Melder_malloc(len);
...
int opus_tags_add(OpusTags *_tags,const char *_tag,const char *_value){
  ...
  comment=(char *)_Melder_malloc(sizeof(*comment)*(tag_len+value_len+2));
...
int opus_tags_set_binary_suffix(OpusTags *_tags,
 const unsigned char *_data,int _len){
  ...
  if(OP_UNLIKELY(ret<0))return ret;
  binary_suffix_data=
   (unsigned char *)Melder_realloc(_tags->user_comments[ncomments],_len);
...
static int opus_picture_tag_parse_impl(OpusPictureTag *_pic,const char *_tag,
 unsigned char *_buf,size_t _buf_sz,size_t _base64_sz){
  ...
  mime_type=(char *)_Melder_malloc(sizeof(*_pic->mime_type)*(mime_type_length+1));
  ...
  description=
   (char *)_Melder_malloc(sizeof(*_pic->mime_type)*(description_length+1));
  ...
  _buf=(unsigned char *)Melder_realloc(_buf,_buf_sz);
...
int opus_picture_tag_parse(OpusPictureTag *_pic,const char *_tag){
  ...
  buf=(unsigned char *)_Melder_malloc(sizeof(*buf)*(buf_sz+1));
...
void opus_picture_tag_clear(OpusPictureTag *_pic){
  Melder_free(_pic->description);
  Melder_free(_pic->mime_type);
  Melder_free(_pic->data);
}
