...
ESPEAK_API int espeak_Initialize(espeak_AUDIO_OUTPUT output_type, int buf_length, const char *path, int options)
{
	...
	case AUDIO_OUTPUT_RETRIEVAL:
		espeak_ng_InitializeOutput((espeak_ng_OUTPUT_MODE) 0, buf_length, NULL);
		break;
	...
	case AUDIO_OUTPUT_SYNCH_PLAYBACK:
		espeak_ng_InitializeOutput((espeak_ng_OUTPUT_MODE) (ENOUTPUT_MODE_SYNCHRONOUS | ENOUTPUT_MODE_SPEAK_AUDIO), buf_length, NULL);
		break;
	}
