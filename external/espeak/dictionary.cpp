...
int LoadDictionary(Translator *tr, const char *name, int no_error)
{
	...
	if (size <= (N_HASH_DICT + sizeof(int)*2)) {
		Melder_error_ (U"Empty _dict: ", Melder_peek8to32(name), U"_dict.");
		return(2);
	}

	if((Reverse4Bytes(pw[0]) != N_HASH_DICT) || (length <= 0) || (length > 0x8000000)) {
		Melder_error_ (U"Bad data in dict: ", Melder_peek8to32(name), U" ", Reverse4Bytes(pw[0]), U" ", length);
		return (2);
	}
	...
	if (tr -> groups1[0] == NULL) {
		Melder_error_ (U"Error in ", Melder_peek8to32(name), U"_rules, no default rule group.");
	}
