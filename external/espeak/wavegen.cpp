...
int PeaksToHarmspect(wavegen_peaks_t *wavegenPeaks, int pitch, int *htab, int control)
{
	if (wvoice == NULL)
		return 1;
	...
	hmax = (wavegenPeaks[wvoice->n_harmonic_peaks].freq + wavegenPeaks[wvoice->n_harmonic_peaks].right)/pitch;
	...
	int h2;
	// increase bass
	y = wavegenPeaks[1].height * 10; // addition as a multiple of 1/256s
...
static int Wavegen(int length, int modulation, bool resume, frame_t *fr1, frame_t *fr2, voice_t *wavegenVoice)
{
	if (resume == false)
		SetSynth(length, modulation, fr1, fr2, wavegenVoice);

	if (wavegenVoice == NULL)
		return 0;
	...
	for (;;) {
		...
		if (wavephase > 0) {
			...
			if (wavephase < 0) {
				...
				for (pk = wavegenVoice->n_harmonic_peaks+1; pk < N_PEAKS; pk++) {
					// find the nearest harmonic for HF peaks where we don't use shape
...
void SetPitch2(voice_t *voicet, int pitch1, int pitch2, int *pitch_base, int *pitch_range)
{
	int base;
	int range;
	int pitch_value;
	...
	base = (voicet->pitch_base * pitch_adjust_tab[pitch_value])/128;
	range =  (voicet->pitch_range * embedded_value[EMBED_R])/50;

	// compensate for change in pitch when the range is narrowed or widened
	base -= (range - voicet->pitch_range)*18;
		...
		case WCMD_MARKER:
			...
			if (marker_type == espeakEVENT_PHONEME) {
				...
				//	MarkerEvent(marker_type, q[1], q[2], q[3], out_ptr);
							// ppgb-espeak: q [3] spilled over when assigning to q [2]
				//else
					MarkerEvent(marker_type, q[1], * (int *) & q[2], * ((int *) & q[2] + 1), out_ptr);
							// ppgb-espeak: split up q [2] into two ints
			}
			break;
		case WCMD_AMPLITUDE:
...
{
	output_hooks = hooks;
	return static_cast<espeak_ng_STATUS> (0);
}
