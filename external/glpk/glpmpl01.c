...
{     int c;
      ...
      {  enter_context(mpl);
         mpl_error(mpl, "control character 0x%02X not allowed", c);
      }
      mpl->c = c;
done: return;
}
...
{     xassert(0 <= mpl->imlen && mpl->imlen <= MAX_LENGTH);
      ...
      {  switch (mpl->token)
         {  case T_NAME:
               enter_context(mpl);
               mpl_error(mpl, "symbolic name %s... too long", mpl->image);
            case T_SYMBOL:
               enter_context(mpl);
               mpl_error(mpl, "symbol %s... too long", mpl->image);
            case T_NUMBER:
               enter_context(mpl);
               mpl_error(mpl, "numeric literal %s... too long", mpl->image);
            case T_STRING:
               enter_context(mpl);
               mpl_error(mpl, "string literal too long");
            default:
               xassert(mpl != mpl);
         }
      }
...
sptp:       {  enter_context(mpl);
               mpl_error(mpl, "keyword s.t. incomplete");
            }
            append_char(mpl);
            if (mpl->c != '.') goto sptp;
            append_char(mpl);
...
frac:       while (isdigit(mpl->c)) append_char(mpl);
         ...
         {  append_char(mpl);
            ...
            if (!isdigit(mpl->c))
            {  enter_context(mpl);
               mpl_error(mpl, "numeric literal %s incomplete", mpl->image);
            }
            while (isdigit(mpl->c)) append_char(mpl);
         }
...
err:     {  enter_context(mpl);
            mpl_error(mpl, "cannot convert numeric literal %s to floating-p"
      ...
      {  /* character string */
         ...
         {  if (mpl->c == '\n' || mpl->c == EOF)
            {  enter_context(mpl);
               mpl_error(mpl, "unexpected end of line; string literal incom"
                  "plete");
            }
            if (mpl->c == quote)
            {  get_char(mpl);
         ...
         {  /* comment sequence */
            ...
            {  if (mpl->c == EOF)
               {  /* do not call enter_context at this point */
                  mpl_error(mpl, "unexpected end of file; comment sequence "
                     "incomplete");
               }
               else if (mpl->c == '*')
               {  get_char(mpl);
      ...
      {  enter_context(mpl);
         mpl_error(mpl, "character %c not allowed", mpl->c);
      }
...
{     ARG_LIST *list;
      ...
      {  /* parse subscript expression */
         ...
         if (x->type != A_SYMBOLIC)
            mpl_error(mpl, "subscript expression has invalid type");
         xassert(x->dim == 0);
         ...
         if (mpl->token == T_COMMA)
            ...
            mpl_error(mpl, "syntax error in subscript list");
      }
      return list;
}
...
{     AVLNODE *node;
      ...
      {  /* subscript list is specified */
         if (dim == 0)
            mpl_error(mpl, "%s cannot be subscripted", name);
         get_token(mpl /* [ */);
         list = subscript_list(mpl);
         if (dim != arg_list_len(mpl, list))
            mpl_error(mpl, "%s must have %d subscript%s rather than %d",
...
{     CODE *x;
      ...
      if (x->type != A_NUMERIC)
         mpl_error(mpl, "argument for %s has invalid type", func);
      xassert(x->dim == 0);
      return x;
}
...
{     CODE *code;
      ...
      {  /* min and max allow arbitrary number of arguments */
         ...
         {  /* parse argument and append it to the operands list */
            ...
            if (mpl->token == T_COMMA)
               ...
               mpl_error(mpl, "syntax error in argument list for %s", func);
         }
      }
      ...
      {  /* atan, round, and trunc need one or two arguments */
         ...
         if (mpl->token == T_COMMA)
            mpl_error(mpl, "%s needs one or two arguments", func);
         else if (mpl->token == T_RIGHT)
            ;
         else
            mpl_error(mpl, "syntax error in argument for %s", func);
      ...
      {  /* substr needs two or three arguments */
         ...
         if (mpl->token == T_COMMA)
            ...
            mpl_error(mpl, "%s needs two or three arguments", func);
         else
            mpl_error(mpl, "syntax error in argument for %s", func);
         get_token(mpl /* , */);
         /* parse the second argument */
      ...
      {  /* other functions need one argument */
         ...
         /* check a token that follows the argument */
         if (mpl->token == T_COMMA)
            mpl_error(mpl, "%s needs one argument", func);
         else if (mpl->token == T_RIGHT)
            ;
         else
            mpl_error(mpl, "syntax error in argument for %s", func);
...
{     CODE *code;
      ...
      for (dim = 1; ; dim++)
      {  if (dim > max_dim)
            mpl_error(mpl, "too many components within parentheses");
         ...
         {  /* symbolic name is recognized as dummy index only if:
            ...
            {  if (list[j].name != NULL && strcmp(list[j].name,
                  mpl->image) == 0)
                  mpl_error(mpl, "duplicate dummy index %s not allowed",
                     mpl->image);
            }
            ...
            if (dim == 1 && mpl->token == T_RIGHT)
               mpl_error(mpl, "%s not defined", list[dim].name);
         }
         else
expr:    {  /* current component of <expression list> is expression */
         ...
         if (mpl->token == T_COMMA)
            ...
            mpl_error(mpl, "right parenthesis missing where expected");
      }
      ...
      {  if (dim == 1)
            ...
            mpl_error(mpl, "0-ary slice not allowed");
      }
      return code;
}
...
{     OPERANDS arg;
      ...
      {  /* all member expressions must be n-tuples; so, if the current
         ...
         if (arg.list != NULL && arg.list->x->dim != code->dim)
            mpl_error(mpl, "member %d has %d component%s while member %d ha"
               "s %d component%s",
         ...
         if (mpl->token == T_COMMA)
            ...
            mpl_error(mpl, "syntax error in literal set");
...
{     DOMAIN *domain;
      ...
      if (mpl->token == T_RBRACE)
         mpl_error(mpl, "empty indexing expression not allowed");
...
expr:    /* parse expression that follows either the keyword 'in' (in
         ...
         {  int dim = 0;
            for (slot = block->list; slot != NULL; slot = slot->next)
               dim++;
            if (dim != code->dim)
               mpl_error(mpl,"%d %s specified for set of dimension %d",
...
{     CODE *code;
      ...
      if (strcmp(mpl->image, "sum") == 0)
         ...
         mpl_error(mpl, "operator %s unknown", mpl->image);
...
err:           mpl_error(mpl, "integrand following %s{...} has invalid type"
                  , opstr);
...
{     CODE *code, *x, *y, *z;
      ...
      if (mpl->token != T_THEN)
         mpl_error(mpl, "keyword then missing where expected");
      get_token(mpl /* then */);
      ...
      if (!(z->type == A_NUMERIC || z->type == A_SYMBOLIC ||
         ...
         mpl_error(mpl, "expression following else has invalid type");
      ...
      if (y->type != z->type)
         mpl_error(mpl, "expressions following then and else have incompati"
            "ble types");
      /* and identical dimensions */
      if (y->dim != z->dim)
         mpl_error(mpl, "expressions following then and else have different"
...
{     CODE *code;
      ...
      {  /* other reserved keywords cannot be used here */
         ...
         mpl_error(mpl, "syntax error in expression");
      return code;
}
...
{     mpl_error(mpl, "operand preceding %s has invalid type", opstr);
      /* no return */
}
...
{     mpl_error(mpl, "operand following %s has invalid type", opstr);
      /* no return */
}
...
{     mpl_error(mpl, "operands preceding and following %s have different di"
      ...
      {  if (mpl->token == T_ASTERISK)
         {  if (x->type == A_SYMBOLIC)
            ...
            if (x->type == A_FORMULA && y->type == A_FORMULA)
               mpl_error(mpl, "multiplication of linear forms not allowed");
      ...
      {  case T_LT:
         ...
         case T_NOT:
            ...
            if (mpl->token == T_IN)
               ...
               mpl_error(mpl, "invalid use of %s", opstr);
            strcat(opstr, " ");
            break;
         default:
            goto done;
...
{     SET *set;
      ...
      {  if (mpl->token == T_COMMA)
         ...
         {  /* dimension of set members */
            ...
            if (set->dimen > 0)
               mpl_error(mpl, "dimension %d conflicts with dimension %d alr"
         ...
         {  /* restricting superset */
            ...
            if (within->code->type != A_ELEMSET)
               mpl_error(mpl, "expression following within has invalid type"
                  );
            ...
            if (set->dimen != within->code->dim)
               mpl_error(mpl, "set expression following within must have di"
...
err:           mpl_error(mpl, "at most one := or default/data allowed");
            ...
            if (set->dimen != set->assign->dim)
               mpl_error(mpl, "set expression following := must have dimens"
                  "ion %d rather than %d",
         ...
         {  /* expression for default value */
            ...
            if (set->option->type != A_ELEMSET)
               mpl_error(mpl, "expression following default has invalid typ"
                  "e");
            xassert(set->option->dim > 0);
            /* check/set dimension of set members */
            if (set->dimen == 0) set->dimen = set->option->dim;
            if (set->dimen != set->option->dim)
               mpl_error(mpl, "set expression following default must have d"
...
{     PARAMETER *par;
      ...
      {  if (mpl->token == T_COMMA)
         ...
         {  if (integer_used)
               mpl_error(mpl, "at most one integer allowed");
            if (par->type == A_SYMBOLIC)
               mpl_error(mpl, "symbolic parameter cannot be integer");
...
bin:     {  if (binary_used)
               mpl_error(mpl, "at most one binary allowed");
            if (par->type == A_SYMBOLIC)
               mpl_error(mpl, "symbolic parameter cannot be binary");
            par->type = A_BINARY;
            binary_used = 1;
         ...
         {  if (symbolic_used)
               mpl_error(mpl, "at most one symbolic allowed");
            ...
            if (!(par->cond == NULL && par->in == NULL &&
               ...
               mpl_error(mpl, "keyword symbolic must precede any other para"
                  "meter attributes");
            par->type = A_SYMBOLIC;
            symbolic_used = 1;
         ...
         {  /* restricting condition */
            ...
            if (!(cond->code->type == A_NUMERIC ||
               ...
               mpl_error(mpl, "expression following %s has invalid type",
                  opstr);
         ...
         {  /* restricting superset */
            ...
            if (in->code->type != A_ELEMSET)
               mpl_error(mpl, "expression following in has invalid type");
            xassert(in->code->dim > 0);
            if (in->code->dim != 1)
               mpl_error(mpl, "set expression following in must have dimens"
...
err:           mpl_error(mpl, "at most one := or default allowed");
         ...
         {  /* expression for default value */
            ...
            mpl_error(mpl, "syntax error in parameter statement");
      }
      /* close the domain scope */
...
{     VARIABLE *var;
      ...
      if (mpl->flag_s)
         mpl_error(mpl, "variable statement must precede solve statement");
      get_token(mpl /* var */);
...
bin:     {  if (binary_used)
         ...
         {  if (!mpl->as_binary)
            ...
            mpl_error(mpl, "variable cannot be symbolic");
         else if (mpl->token == T_GE)
         {  /* lower bound */
            if (var->lbnd != NULL)
            {  if (var->lbnd == var->ubnd)
                  mpl_error(mpl, "both fixed value and lower bound not allo"
                     "wed");
               else
                  mpl_error(mpl, "at most one lower bound allowed");
            }
            get_token(mpl /* >= */);
         ...
         {  /* upper bound */
            ...
            {  if (var->ubnd == var->lbnd)
                  mpl_error(mpl, "both fixed value and upper bound not allo"
                     "wed");
               else
                  mpl_error(mpl, "at most one upper bound allowed");
            }
            get_token(mpl /* <= */);
         ...
         {  /* fixed value */
            ...
            {  if (var->lbnd == var->ubnd)
                  mpl_error(mpl, "at most one fixed value allowed");
               else if (var->lbnd != NULL)
                  mpl_error(mpl, "both lower bound and fixed value not allo"
                     "wed");
               else
                  mpl_error(mpl, "both upper bound and fixed value not allo"
                     "wed");
            }
            strcpy(opstr, mpl->image);
            xassert(strlen(opstr) < sizeof(opstr));
            ...
            var->ubnd = var->lbnd;
                  ...
                  mpl->token == T_NE)
            mpl_error(mpl, "strict bound not allowed");
         else
            mpl_error(mpl, "syntax error in variable statement");
      }
      /* close the domain scope */
...
{     CONSTRAINT *con;
      ...
      char opstr[8];
      if (mpl->flag_s)
         mpl_error(mpl, "constraint statement must precede solve statement")
            ;
      if (is_keyword(mpl, "subject"))
      {  get_token(mpl /* subject */);
         if (!is_keyword(mpl, "to"))
            mpl_error(mpl, "keyword subject to incomplete");
         get_token(mpl /* to */);
      }
      else if (is_keyword(mpl, "subj"))
      {  get_token(mpl /* subj */);
         if (!is_keyword(mpl, "to"))
            mpl_error(mpl, "keyword subj to incomplete");
         get_token(mpl /* to */);
      }
      ...
      if (!(second->type == A_NUMERIC || second->type == A_FORMULA))
         mpl_error(mpl, "expression following %s has invalid type", opstr);
      ...
      {  /* it is another relational operator, therefore the constraint
         ...
         if (first->type == A_FORMULA)
            mpl_error(mpl, "leftmost expression in double inequality cannot"
               " be linear form");
         get_token(mpl /* rho */);
         ...
         if (!(third->type == A_NUMERIC || third->type == A_FORMULA))
            mpl_error(mpl, "rightmost expression in double inequality const"
               "raint has invalid type");
         xassert(third->dim == 0);
         /* the third expression also cannot be linear form */
         if (third->type == A_FORMULA)
            mpl_error(mpl, "rightmost expression in double inequality canno"
               "t be linear form");
      }
      else
...
err:     mpl_error(mpl, "syntax error in constraint statement");
      get_token(mpl /* ; */);
      return con;
}
...
{     CONSTRAINT *obj;
      ...
      if (is_keyword(mpl, "minimize"))
         ...
         xassert(mpl != mpl);
      if (mpl->flag_s)
         mpl_error(mpl, "objective statement must precede solve statement");
      ...
      if (mpl->token != T_SEMICOLON)
         mpl_error(mpl, "syntax error in objective statement");
      get_token(mpl /* ; */);
      return obj;
}
...
{     TABLE *tab;
      ...
      {  /* this is output table */
         ...
         if (!is_keyword(mpl, "OUT"))
            mpl_error(mpl, "keyword OUT missing where expected");
         get_token(mpl /* OUT */);
      }
      else
      {  /* this is input table */
      ...
      {  /* create argument list entry */
         ...
         if (mpl->token == T_COMMA || mpl->token == T_COLON ||
            ...
            mpl_error(mpl, "argument expression missing where expected");
         ...
         if (arg->code->type != A_SYMBOLIC)
            mpl_error(mpl, "argument expression has invalid type");
...
input_table:
      ...
      {  node = avl_find_node(mpl->tree, mpl->image);
         if (node == NULL)
            mpl_error(mpl, "%s not defined", mpl->image);
         if (avl_get_node_type(node) != A_SET)
            mpl_error(mpl, "%s not a set", mpl->image);
         ...
         if (mpl->token == T_INPUT)
            ...
            mpl_error(mpl, "delimiter <- missing where expected");
      }
      else if (is_reserved(mpl))
         mpl_error(mpl, "invalid use of reserved keyword %s", mpl->image);
      else
      ...
      if (mpl->token == T_LBRACKET)
         ...
         mpl_error(mpl, "field list missing where expected");
      for (;;)
      {  /* create field list entry */
         ...
         if (mpl->token == T_NAME)
            ...
            mpl_error(mpl, "field name missing where expected");
         ...
         if (mpl->token == T_COMMA)
            ...
            mpl_error(mpl, "syntax error in field list");
      }
      ...
      {  get_token(mpl /* , */);
         ...
         if (avl_get_node_type(node) != A_PARAMETER)
            mpl_error(mpl, "%s not a parameter", mpl->image);
...
output_table:
      ...
      {  /* create output list entry */
         ...
         if (mpl->token == T_COMMA || mpl->token == T_SEMICOLON)
            mpl_error(mpl, "expression missing where expected");
         if (mpl->token == T_NAME)
         {  xassert(strlen(mpl->image) < sizeof(name));
            strcpy(name, mpl->image);
         }
         ...
         /* assign field name */
         if (name[0] == '\0')
            mpl_error(mpl, "field name required");
         ...
         if (mpl->token == T_COMMA)
            ...
            mpl_error(mpl, "syntax error in output list");
      }
      /* close the domain scope */
...
end_of_table:
      ...
      if (mpl->token != T_SEMICOLON)
         mpl_error(mpl, "syntax error in table statement");
      get_token(mpl /* ; */);
      return tab;
}
#endif
...
{     xassert(is_keyword(mpl, "solve"));
      if (mpl->flag_s)
         mpl_error(mpl, "at most one solve statement allowed");
      mpl->flag_s = 1;
      get_token(mpl /* solve */);
      /* semicolon must follow solve statement */
      if (mpl->token != T_SEMICOLON)
         mpl_error(mpl, "syntax error in solve statement");
      get_token(mpl /* ; */);
      return NULL;
}
...
expr:    {  /* display entry is expression */
      ...
      if (mpl->token != T_SEMICOLON)
         mpl_error(mpl, "syntax error in display statement");
      get_token(mpl /* ; */);
      return dpy;
}
#endif
...
{     PRINTF *prt;
      ...
      if (prt->fmt->type != A_SYMBOLIC)
         mpl_error(mpl, "format expression has invalid type");
      /* parse printf list */
      while (mpl->token == T_COMMA)
      {  get_token(mpl /* , */);
         ...
         if (!(entry->code->type == A_NUMERIC ||
            ...
            mpl_error(mpl, "only numeric, symbolic, or logical expression a"
               "llowed");
      }
      /* close the domain scope */
      ...
      {  prt->app = (mpl->token == T_APPEND);
         ...
         if (prt->fname->type != A_SYMBOLIC)
            mpl_error(mpl, "file name expression has invalid type");
      }
#endif
      /* the printf statement has been completely parsed */
      if (mpl->token != T_SEMICOLON)
         mpl_error(mpl, "syntax error in printf statement");
      get_token(mpl /* ; */);
      return prt;
}
...
{     FOR *fur;
      ...
      if (mpl->token != T_LBRACE)
         mpl_error(mpl, "indexing expression missing where expected");
...
{     STATEMENT *stmt;
      ...
      {  if (spec)
            mpl_error(mpl, "set statement not allowed here");
         stmt->type = A_SET;
         stmt->u.set = set_statement(mpl);
      }
      else if (is_keyword(mpl, "param"))
      {  if (spec)
            mpl_error(mpl, "parameter statement not allowed here");
         stmt->type = A_PARAMETER;
         stmt->u.par = parameter_statement(mpl);
      }
      else if (is_keyword(mpl, "var"))
      {  if (spec)
            mpl_error(mpl, "variable statement not allowed here");
         ...
         stmt->u.var = variable_statement(mpl);
               ...
               mpl->token == T_SPTP)
      {  if (spec)
            mpl_error(mpl, "constraint statement not allowed here");
      ...
      {  if (spec)
            mpl_error(mpl, "objective statement not allowed here");
         stmt->type = A_CONSTRAINT;
      ...
      {  if (spec)
            mpl_error(mpl, "table statement not allowed here");
         stmt->type = A_TABLE;
      ...
      {  if (spec)
            mpl_error(mpl, "solve statement not allowed here");
         stmt->type = A_SOLVE;
      ...
      {  if (spec)
         ...
         mpl_error(mpl, "syntax error in model section");
      return stmt;
}
