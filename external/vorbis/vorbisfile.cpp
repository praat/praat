...
static void _add_serialno(ogg_page *og,long **serialno_list, int *n){
  ...
  if(*serialno_list){
    *serialno_list = (long *) Melder_realloc (*serialno_list, sizeof(**serialno_list)*(*n));
  }else{
    *serialno_list = (long *) _Melder_malloc(sizeof(**serialno_list));
  }
  ...
  while(ogg_page_bos(og_ptr)){
    if(serialno_list){
      if(_lookup_page_serialno(og_ptr,*serialno_list,*serialno_n)){
        ...
        if(*serialno_list)Melder_free(*serialno_list);
        *serialno_list=0;
        *serialno_n=0;
        ret=OV_EBADHEADER;
        goto bail_header;
...
static int _bisect_forward_serialno(OggVorbis_File *vf,
  ...
  */
  ...
  if(_lookup_serialno(endserial,currentno_list,currentnos)){
    ...
    vf->links=m+1;
    if(vf->offsets)Melder_free(vf->offsets);
    if(vf->serialnos)Melder_free(vf->serialnos);
    if(vf->dataoffsets)Melder_free(vf->dataoffsets);

    vf->offsets=(ogg_int64_t *)_Melder_malloc((vf->links+1)*sizeof(*vf->offsets));
    vf->vi=(vorbis_info *)Melder_realloc(vf->vi,vf->links*sizeof(*vf->vi));
    vf->vc=(vorbis_comment *)Melder_realloc(vf->vc,vf->links*sizeof(*vf->vc));
    vf->serialnos=(long *)_Melder_malloc(vf->links*sizeof(*vf->serialnos));
    vf->dataoffsets=(ogg_int64_t *)_Melder_malloc(vf->links*sizeof(*vf->dataoffsets));
    vf->pcmlengths=(ogg_int64_t *)_Melder_malloc(vf->links*2*sizeof(*vf->pcmlengths));
    ...
    long *next_serialno_list=nullptr;
    ...
    ret=_fetch_headers(vf,&vi,&vc,&next_serialno_list,&next_serialnos,nullptr);
    if(ret)return(ret);
    ...
    if(next_serialno_list)Melder_free(next_serialno_list);
        ...
        if(result>0){
          ...
          if(!vorbis_synthesis(&vf->vb,op_ptr)){ /* lazy check for lazy
            ...
            /* suck in the synthesis data and track bitrate */
            {
              ...
              vf->samptrack+=(vorbis_synthesis_pcmout(&vf->vd,nullptr)<<hs);
            ...
            if(granulepos!=-1 && !op_ptr->e_o_s){
              ...
              samples=(vorbis_synthesis_pcmout(&vf->vd,nullptr)<<hs);
    ...
    if(vf->ready_state!=INITSET){
      ...
      if(vf->ready_state<STREAMSET){
        if(vf->seekable){
          ...
          int ret=_fetch_headers(vf,vf->vi,vf->vc,nullptr,nullptr,&og);
          if(ret)return(ret);
...
static int _ov_open1(void *f,OggVorbis_File *vf,const char *initial,
  ...
  vf->links=1;
  vf->vi=(vorbis_info *)_Melder_calloc(vf->links,sizeof(*vf->vi));
  vf->vc=(vorbis_comment *)_Melder_calloc(vf->links,sizeof(*vf->vc));
  ogg_stream_init(&vf->os,-1); /* fill in the serialno later */

  /* Fetch all BOS pages, store the vorbis header and all seen serial
     numbers, load subsequent vorbis setup headers */
  if((ret=_fetch_headers(vf,vf->vi,vf->vc,&serialno_list,&serialno_list_size,nullptr))<0){
    ...
    vf->serialnos=(long *)_Melder_calloc(serialno_list_size+2,sizeof(*vf->serialnos));
    ...
    vf->offsets=(ogg_int64_t *)_Melder_calloc(1,sizeof(*vf->offsets));
    vf->dataoffsets=(ogg_int64_t *)_Melder_calloc(1,sizeof(*vf->dataoffsets));
    vf->offsets[0]=0;
...
int ov_clear(OggVorbis_File *vf){
  if(vf){
    ...
    if(vf->dataoffsets)Melder_free(vf->dataoffsets);
    if(vf->pcmlengths)Melder_free(vf->pcmlengths);
    if(vf->serialnos)Melder_free(vf->serialnos);
    if(vf->offsets)Melder_free(vf->offsets);
...
long ov_bitrate(OggVorbis_File *vf,int i){
  ...
  if(i<0){
    ...
    //int i;
    float br;
    for(integer k=0;k<vf->links;k++)
      bits+=(vf->offsets[k+1]-vf->dataoffsets[k])*8;
...
long ov_read(OggVorbis_File *vf,char *buffer,int length,
             int bigendianp,int word,int sgned,int *bitstream){
  return ov_read_filter(vf, buffer, length, bigendianp, word, sgned, bitstream, nullptr, nullptr);
}
