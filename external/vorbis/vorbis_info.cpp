...
void vorbis_comment_add(vorbis_comment *vc,const char *comment){
  vc->user_comments=(char **)Melder_realloc(vc->user_comments,
                            (vc->comments+2)*sizeof(*vc->user_comments));
  vc->comment_lengths=(int *)Melder_realloc(vc->comment_lengths,
...
void vorbis_comment_add_tag(vorbis_comment *vc, const char *tag, const char *contents){
  ...
  char *comment=(char *)_Melder_malloc(strlen(tag)+strlen(contents)+2);
  strcpy(comment, tag);
  strcat(comment, "=");
...
char *vorbis_comment_query(vorbis_comment *vc, const char *tag, int count){
  ...
  char *fulltag = (char *)_Melder_malloc(taglen+1);
...
void vorbis_comment_clear(vorbis_comment *vc){
  if(vc){
    ...
    if(vc->user_comments){
      for(i=0;i<vc->comments;i++)
        if(vc->user_comments[i])Melder_free(vc->user_comments[i]);
      Melder_free(vc->user_comments);
    }
    if(vc->comment_lengths)Melder_free(vc->comment_lengths);
    if(vc->vendor)Melder_free(vc->vendor);
    memset(vc,0,sizeof(*vc));
  }
}
...
void vorbis_info_init(vorbis_info *vi){
  ...
  vi->codec_setup=_Melder_calloc(1,sizeof(codec_setup_info));
}
...
void vorbis_info_clear(vorbis_info *vi){
  ...
  if(ci){
    ...
    for(i=0;i<ci->modes;i++)
      if(ci->mode_param[i])Melder_free(ci->mode_param[i]);
...
static int _vorbis_unpack_comment(vorbis_comment *vc,oggpack_buffer *opb){
  ...
  vc->vendor = (char *) _Melder_calloc(vendorlen+1,1);
  _v_readstring(opb,vc->vendor,vendorlen);
  i=oggpack_read(opb,32);
  if(i<0)goto err_out;
  if(i>((opb->storage-oggpack_bytes(opb))>>2))goto err_out;
  vc->comments=i;
  vc->user_comments = (char **) _Melder_calloc(vc->comments+1,sizeof(*vc->user_comments));
  vc->comment_lengths = (int *) _Melder_calloc(vc->comments+1, sizeof(*vc->comment_lengths));
  ...
  for(i=0;i<vc->comments;i++){
    ...
    vc->user_comments[i] = (char *)_Melder_calloc(len+1,1);
...
static int _vorbis_unpack_books(vorbis_info *vi,oggpack_buffer *opb){
  ...
  for(i=0;i<ci->modes;i++){
    ci->mode_param[i] = (vorbis_info_mode *) _Melder_calloc(1,sizeof(*ci->mode_param[i]));
...
int vorbis_commentheader_out(vorbis_comment *vc,
  ...
  op->packet = (unsigned char *) _Melder_malloc(oggpack_bytes(&opb));
...
int vorbis_analysis_headerout(vorbis_dsp_state *v,
  ...
  b->header = (unsigned char *) _Melder_malloc(oggpack_bytes(&opb));
  ...
  b->header1 = (unsigned char *)_Melder_malloc(oggpack_bytes(&opb));
  ...
  b->header2 = (unsigned char *)_Melder_malloc(oggpack_bytes(&opb));
