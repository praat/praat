    ...
    for(i=0;i<PACKETBLOBS;i++){
      if(i==PACKETBLOBS/2){
        ...
        vbi->packetblob[i]= (oggpack_buffer *)
          _Melder_calloc(1,sizeof(oggpack_buffer));
      }
...
void *_vorbis_block_alloc(vorbis_block *vb,long bytes){
  ...
  if(bytes+vb->localtop>vb->localalloc){
    ...
    if(vb->localstore){
      struct alloc_chain *link = (struct alloc_chain *) _Melder_malloc(sizeof(*link));
    ...
    /* highly conservative */
    vb->localalloc=bytes;
    vb->localstore=_Melder_malloc(vb->localalloc);
    vb->localtop=0;
  }
  {
...
void _vorbis_block_ripcord(vorbis_block *vb){
  ...
  /* consolidate storage */
  if(vb->totaluse){
    vb->localstore= Melder_realloc(vb->localstore,vb->totaluse+vb->localalloc);
...
int vorbis_block_clear(vorbis_block *vb){
  ...
  if(vb->localstore) Melder_free(vb->localstore);

  if(vbi){
    for(i=0;i<PACKETBLOBS;i++){
      oggpack_writeclear(vbi->packetblob[i]);
      if(i!=PACKETBLOBS/2)Melder_free(vbi->packetblob[i]);
    }
    Melder_free(vbi);
  }
  memset(vb,0,sizeof(*vb));
...
static int _vds_shared_init(vorbis_dsp_state *v,vorbis_info *vi,int encp){
  ...
  memset(v,0,sizeof(*v));
  v->backend_state = (private_state *) _Melder_calloc(1,sizeof(*b));
  b = (private_state *) v->backend_state;

  v->vi=vi;
  b->modebits=ov_ilog(ci->modes-1);

  b->transform[0] = (vorbis_look_transform **) _Melder_calloc(VI_TRANSFORMB,sizeof(*b->transform[0]));
  b->transform[1] = (vorbis_look_transform **) _Melder_calloc(VI_TRANSFORMB,sizeof(*b->transform[1]));

  /* MDCT is tranform 0 */

  b->transform[0][0]=_Melder_calloc(1,sizeof(mdct_lookup));
  b->transform[1][0]=_Melder_calloc(1,sizeof(mdct_lookup));
  ...
  if(encp){ /* encode/decode differ here */
    ...
    b->psy = (vorbis_look_psy *)_Melder_calloc(ci->psys,sizeof(*b->psy));
    for(i=0;i<ci->psys;i++){
...
int vorbis_analysis_init(vorbis_dsp_state *v,vorbis_info *vi){
  ...
  b->ve= (envelope_lookup *) _Melder_calloc(1,sizeof(*b->ve));
  ...
  if(v){
    ...
    if(b){
      ...
      if(b->transform[0]){
        ...
        Melder_free(b->transform[0][0]);
        Melder_free(b->transform[0]);
      }
      if(b->transform[1]){
        mdct_clear((mdct_lookup *) b->transform[1][0]);
        Melder_free(b->transform[1][0]);
        Melder_free(b->transform[1]);
      }
    ...
    if(v->pcm){
      if(vi)
        for(i=0;i<vi->channels;i++)
          if(v->pcm[i])Melder_free(v->pcm[i]);
      Melder_free(v->pcm);
      if(v->pcmret)Melder_free(v->pcmret);
    }
...
float **vorbis_analysis_buffer(vorbis_dsp_state *v, int vals){
  ...
  if(v->pcm_current+vals>=v->pcm_storage){
    ...
    for(i=0;i<vi->channels;i++){
      v->pcm[i]= (float *) Melder_realloc(v->pcm[i],v->pcm_storage*sizeof(*v->pcm[i]));
    }
  }
