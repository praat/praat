...
ogg_uint32_t *_make_words(char *l,long n,long sparsecount){
  ...
  ogg_uint32_t *r = (ogg_uint32_t *)_Melder_malloc((sparsecount?sparsecount:n)*sizeof(*r));
...
float *_book_unquantize(const static_codebook *b,int n,int *sparsemap){
  ...
  if(b->maptype==1 || b->maptype==2){
    ...
    float *r = (float *) _Melder_calloc(n*b->dim,sizeof(*r));
...
void vorbis_staticbook_destroy(static_codebook *b){
  if(b->allocedp){
    if(b->quantlist)Melder_free(b->quantlist);
    if(b->lengthlist)Melder_free(b->lengthlist);
    memset(b,0,sizeof(*b));
    Melder_free(b);
...
void vorbis_book_clear(codebook *b){
  ...
  if(b->valuelist)Melder_free(b->valuelist);
  if(b->codelist)Melder_free(b->codelist);

  if(b->dec_index)Melder_free(b->dec_index);
  if(b->dec_codelengths)Melder_free(b->dec_codelengths);
  if(b->dec_firsttable)Melder_free(b->dec_firsttable);
...
int vorbis_book_init_decode(codebook *c,const static_codebook *s){
  ...
  if(n>0){
    ...
    c->codelist= (ogg_uint32_t *) _Melder_malloc(n*sizeof(*c->codelist));
    ...
    c->dec_index = (int *) _Melder_malloc(n*sizeof(*c->dec_index));

    for(n=0,i=0;i<s->entries;i++)
      if(s->lengthlist[i]>0)
        c->dec_index[sortindex[n++]]=i;

    c->dec_codelengths = (char *) _Melder_malloc(n*sizeof(*c->dec_codelengths));
    c->dec_maxlength=0;
    for(n=0,i=0;i<s->entries;i++)
      if(s->lengthlist[i]>0){
    ...
    if(n==1 && c->dec_maxlength==1){
      ...
      c->dec_firsttablen=1;
      c->dec_firsttable= (ogg_uint32_t *) _Melder_calloc(2,sizeof(*c->dec_firsttable));
      ...
      c->dec_firsttable = (ogg_uint32_t *) _Melder_calloc(tabn,sizeof(*c->dec_firsttable));
