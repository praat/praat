...
vorbis_look_psy_global *_vp_global_look(vorbis_info *vi){
  ...
  vorbis_look_psy_global *look = (vorbis_look_psy_global *)_Melder_calloc(1,sizeof(*look));
...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS],float binHz,int n,
  ...
  float ***ret = (float ***) _Melder_malloc(sizeof(*ret)*P_BANDS);
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    ret[i] = (float **) _Melder_malloc(sizeof(**ret)*P_LEVELS);
    ...
    for(m=0;m<P_LEVELS;m++){
      ret[i][m] = (float *) _Melder_malloc(sizeof(***ret)*(EHMER_MAX+2));
      ...
      for(j=0;j<EHMER_MAX;j++){
        int binn=fromOC(j*.125+i*.5-2.)/binHz;
        if(binn<0){
          ret[i][m][j+2]=-999.;
        }else{
          if(binn>=n){
...
void _vp_psy_init(vorbis_look_psy *p,vorbis_info_psy *vi,
  ...
  p->ath= (float *) _Melder_malloc(n*sizeof(*p->ath));

  p->octave = (long *)_Melder_malloc(n*sizeof(*p->octave));
  p->bark = (long *) _Melder_malloc(n*sizeof(*p->bark));
  p->vi=vi;
  p->n=n;
  p->rate=rate;
  ...
  p->noiseoffset = (float **) _Melder_malloc(P_NOISECURVES*sizeof(*p->noiseoffset));
  for(i=0;i<P_NOISECURVES;i++)
    p->noiseoffset[i]= (float *) _Melder_malloc(n*sizeof(**p->noiseoffset));
...
void _vp_psy_clear(vorbis_look_psy *p){
  int i,j;
  if(p){
    if(p->ath)Melder_free(p->ath);
    if(p->octave)Melder_free(p->octave);
    if(p->bark)Melder_free(p->bark);
    if(p->tonecurves){
      for(i=0;i<P_BANDS;i++){
        for(j=0;j<P_LEVELS;j++){
          Melder_free(p->tonecurves[i][j]);
        }
        Melder_free(p->tonecurves[i]);
      }
      Melder_free(p->tonecurves);
    }
    if(p->noiseoffset){
      for(i=0;i<P_NOISECURVES;i++){
        Melder_free(p->noiseoffset[i]);
      }
      Melder_free(p->noiseoffset);
    }
    memset(p,0,sizeof(*p));
  }
}
...
void _vp_offset_and_mix(vorbis_look_psy *p,
  ...
  for(i=0;i<n;i++){
    ...
    logmask[i] = std::max(val,tone[i]+toneatt);
