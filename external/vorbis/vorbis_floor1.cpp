...
static vorbis_info_floor *floor1_unpack (vorbis_info *vi,oggpack_buffer *opb){
  ...
  vorbis_info_floor1 *info= (vorbis_info_floor1 *) _Melder_calloc(1,sizeof(*info));
  /* read partitions */
...
static vorbis_look_floor *floor1_look(vorbis_dsp_state *vd,
  ...
  vorbis_look_floor1 *look= (vorbis_look_floor1 *) _Melder_calloc(1,sizeof(*look));
  int i,j,n=0;
...
int floor1_encode(oggpack_buffer *opb,vorbis_block *vb,
  ...
  if(post){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      int classs=info->partitionclass[i];
      int cdim=info->class_dim[classs];
      int csubbits=info->class_subs[classs];
      int csub=1<<csubbits;
      ...
      if(csubbits){
        int maxval[8]={0,0,0,0,0,0,0,0}; /* gcc's static analysis
                                            ...
                                            initialization */
        for(k=0;k<csub;k++){
          int booknum=info->class_subbook[classs][k];
          if(booknum<0){
            maxval[k]=1;
          }else{
            maxval[k]=sbooks[info->class_subbook[classs][k]]->entries;
          }
        }
        for(k=0;k<cdim;k++){
          for(l=0;l<csub;l++){
        ...
        /* write it */
        look->phrasebits+=
          vorbis_book_encode(books+info->class_book[classs],cval,opb);
      ...
      /* write post values */
      for(k=0;k<cdim;k++){
        int book=info->class_subbook[classs][bookas[k]];
        if(book>=0){
...
#ifdef TRAIN_FLOOR1
          {
            ...
            sprintf(buffer,"line_%dx%ld_%dsub%d.vqd",
                    vb->pcmend/2,posts-2,classs,bookas[k]);
            of=fopen(buffer,"a");
            fprintf(of,"%d\n",out[j+k]);
            fclose(of);
          }
...
static void *floor1_inverse1(vorbis_block *vb,vorbis_look_floor *in){
  ...
  if(oggpack_read(&vb->opb,1)==1){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
        cval=vorbis_book_decode(books+info->class_book[classs],&vb->opb);

        if(cval==-1)goto eop;
      }

      for(k=0;k<cdim;k++){
        int book=info->class_subbook[classs][cval&(csub-1)];
        cval>>=csubbits;
        if(book>=0){
