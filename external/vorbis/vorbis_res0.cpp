...
void res0_free_look(vorbis_look_residue *i){
  ...
  if(i){
    ...
    for(j=0;j<look->parts;j++)
      if(look->partbooks[j])Melder_free(look->partbooks[j]);
    Melder_free(look->partbooks);
    for(j=0;j<look->partvals;j++)
      Melder_free(look->decodemap[j]);
    Melder_free(look->decodemap);
...
vorbis_info_residue *res0_unpack(vorbis_info *vi,oggpack_buffer *opb){
  int j,acc=0;
  vorbis_info_residue0 *info = (vorbis_info_residue0 *) _Melder_calloc(1,sizeof(*info));
...
vorbis_look_residue *res0_look(vorbis_dsp_state *vd,
  ...
  vorbis_info_residue0 *info=(vorbis_info_residue0 *)vr;
  vorbis_look_residue0 *look= (vorbis_look_residue0 *) _Melder_calloc(1,sizeof(*look));
  ...
  look->partbooks= (codebook ***) _Melder_calloc(look->parts,sizeof(*look->partbooks));

  for(j=0;j<look->parts;j++){
    int stages=ov_ilog(info->secondstages[j]);
    if(stages){
      if(stages>maxstage)maxstage=stages;
      look->partbooks[j] = (codebook **) _Melder_calloc(stages,sizeof(*look->partbooks[j]));
      for(k=0;k<stages;k++)
  ...
  look->decodemap = (int **) _Melder_malloc(look->partvals*sizeof(*look->decodemap));
  for(j=0;j<look->partvals;j++){
    long val=j;
    long mult=look->partvals/look->parts;
    look->decodemap[j] = (int *) _Melder_malloc(dim*sizeof(*look->decodemap[j]));
    for(k=0;k<dim;k++){
      long deco=val/mult;
      val-=deco*mult;
      mult/=look->parts;
