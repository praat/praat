...
int vorbis_staticbook_pack(const static_codebook *c,oggpack_buffer *opb){
  ...
  if(ordered){
    ...
    for(i=1;i<c->entries;i++){
      char thiss=c->lengthlist[i];
      char last=c->lengthlist[i-1];
      if(thiss>last){
        for(j=last;j<thiss;j++){
...
static_codebook *vorbis_staticbook_unpack(oggpack_buffer *opb){
  long i,j;
  static_codebook *s = (static_codebook *) _Melder_calloc(1,sizeof(*s));
  s->allocedp=1;
  ...
  case 0:{
    ...
    if((s->entries*(unused?1:5)+7)>>3>opb->storage-oggpack_bytes(opb))
      goto _eofout;
    /* unordered */
    s->lengthlist = (char *) _Melder_malloc(sizeof(*s->lengthlist)*s->entries);
  ...
  case 1: case 2:
...

    {
      ...
      if(((quantvals*s->q_quant+7)>>3)>opb->storage-oggpack_bytes(opb))
        goto _eofout;
      s->quantlist= (long *) _Melder_malloc(sizeof(*s->quantlist)*quantvals);
      for(i=0;i<quantvals;i++)
...
inline long decode_packed_entry_number(codebook *book, oggpack_buffer *b){
