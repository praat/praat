...
MAN_BEGIN (U"Functions", U"ppgb", 20211115)
...
LIST_ITEM (U"##index (%%strings\\$ \\# %, %%s\\$ %)# \\-- look up whether and where %%s\\$ % occurs in %%strings\\$ \\# %")
...
LIST_ITEM (U"##percent\\$  (%number, %precision)# \\-- format a number as a string, "
	"with a training percent sign and %precision digits after the decimal point")
