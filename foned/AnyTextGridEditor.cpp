...
void structAnyTextGridEditor :: v_draw () {
	...
	*/
	...
	if (isdefined (our textGridArea() -> draggingTime) && hasBeenDraggedBeyondVicinityRadiusAtLeastOnce) {
		FunctionArea_setViewport (our textGridArea().get());
		Graphics_xorOn (our graphics.get(), Melder_MAROON);
		for (integer itier = 1; itier <= numberOfTiers; itier ++) {
			if (our textGridArea() -> draggingTiers [itier]) {
				...
				Graphics_line (our graphics.get(), our textGridArea() -> draggingTime, ymin, our textGridArea() -> draggingTime, ymax);
			}
		}
		our viewDataAsWorldByFraction ();
		Graphics_setLineWidth (our graphics.get(), 1.0);
		Graphics_line (our graphics.get(), our textGridArea() -> draggingTime, 0.0, our textGridArea() -> draggingTime, 1.0);
		Graphics_text (our graphics.get(), our textGridArea() -> draggingTime, 1.0, Melder_fixed (our textGridArea() -> draggingTime, 6));
	...
	SpellingChecker spellingChecker, conststring32 callbackSocket)
{
	...
	my textGridArea() -> draggingTime = undefined;
