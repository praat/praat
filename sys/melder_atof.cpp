// melder_atof.cpp
// Copyright 2003-2018 Paul Boersma
// This code is distributed under GPL 3 or later (see ../LICENSE.TXT).

#include "melder.h"

/// Assume that the next thing that follows is a numeric string, and find the
/// end of it. Return null on error.
template <typename T>
static const T *findEndOfNumericString (const T *string) noexcept {
	const T *p = &string[0];
	// Leading white space is OK.
	while (Melder_isAsciiHorizontalOrVerticalSpace (*p)) ++p;
	...
	if (*p == '.') {
		++p;
		// We accept any number of (even zero) decimal digits after the decimal
		// point.
		while (Melder_isAsciiDecimalNumber (*p)) ++p;
	}
...
double Melder_a8tof (conststring8 string) noexcept {
	...
	return p[-1] == '%' ? 0.01 * strtod (string, nullptr)
	                    : strtod (string, nullptr);
}
