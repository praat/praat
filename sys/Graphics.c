	...
	short x1DC, short x2DC, short y1DC, short y2DC)
{
	...
	if (x1DC < my x1DCmin || x2DC > my x2DCmax || y1DC < my y1DCmin || y2DC > my y2DCmax) {
		Melder_warning ("Graphics_setWsViewport: coordinates too large:\n"
			"%d..%d x %d..%d goes outside %d..%d x %d..%d.",
			x1DC, x2DC, y1DC, y2DC, my x1DCmin, my x2DCmax, my y1DCmin, my y2DCmax);
		x1DC = my x1DCmin;
		x2DC = my x2DCmax;
		y1DC = my y1DCmin;
		y2DC = my y2DCmax;
