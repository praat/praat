...
#define oo_VECTOR(type,t,storage,x,min,max)  \
	if (max >= min && ! (my x = NUM##t##vector_readAscii (min, max, f, #x))) return 0;

#define oo_MATRIX(type,t,storage,x,row1,row2,col1,col2)  \
	if (row2 >= row1 && col2 >= col1 && \
	    ! (my x = NUM##t##matrix_readAscii (row1, row2, col1, col2, f, #x))) return 0;

#define oo_ENUMx(type,storage,Type,x)  \
	if ((my x = ascget##storage (f, & enum_##Type)) < 0) return 0;
...
#define oo_ENUMx_ARRAY(type,storage,Type,x,cap,n)  \
	...
	{ \
		long i; \
		for (i = 0; i < n; i ++) \
			if ((my x [i] = ascget##storage (f, & enum_##Type)) < 0) return 0; \
	}
...
#define oo_STRUCT(Type,x)  \
	if (! Type##_readAscii (& my x, f)) return 0;
...
#define oo_STRUCT_ARRAY(Type,x,cap,n) \
	...
	{ \
		long i; \
		for (i = 0; i < n; i ++) \
			if (! Type##_readAscii (& my x [i], f)) return 0; \
	}
...
#define oo_OBJECT(Class,x)  \
	if (ascgetex (f)) { if (! (my x = new (Class)) || ! Data_readAscii (my x, f)) return 0; }
...
#define oo_COLLECTION(Class,x,ItemClass)  \
	{ \
		...
		for (i = 1; i <= n; i ++) { \
			...
			if (! item || ! item -> methods -> readAscii (item, f)) return 0; \
...
#define oo_DEFINE_STRUCT(Type)  \
	static int Type##_readAscii (Type me, FILE *f) { \
